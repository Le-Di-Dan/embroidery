...
const expectedFile =
  process.argv[3] ?? join(dirname(fileURLToPath(import.meta.url)), 'canonical-fingerprint.txt');
...
let actual;
try {
  actual = execFileSync('node', [scriptPath, url], {
    encoding: 'utf8',
...
if (actual !== expected) {
  console.error(
    '[fingerprint-gate] MISMATCH — live schema fingerprint differs from the canonical baseline',
  );
  console.error(`[fingerprint-gate] expected: ${expected}`);
  console.error(`[fingerprint-gate] actual:   ${actual}`);
  console.error(
    '[fingerprint-gate] this means an object changed since the baseline was set — identify the exact',
  );
  console.error(
    '[fingerprint-gate] table/constraint/index/function/trigger, do not update this file to force a pass.',
  );
  process.exit(1);
}
