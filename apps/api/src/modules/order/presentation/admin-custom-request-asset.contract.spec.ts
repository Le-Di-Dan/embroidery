...
describe('APP5-B06 — the published Admin request-asset contract', () => {
  ...
  it('is the only Admin address that serves bytes, and adds no generic asset route', () => {
    ...
    // Two, and the other one predates this checkpoint: `APP3-A01`'s placement
    // background, which is contextual to a product side in exactly the same way
    // this is contextual to a request. B06 adds the second and no third — no
    // generic binary, and no companion "download" or "thumbnail" address.
