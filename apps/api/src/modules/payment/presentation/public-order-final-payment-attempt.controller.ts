/**
 * The one customer final-payment write (`APP9-B02`).
 *
 * ```text
 ...
 * transaction under its row lock (`ADR-DB3-004` r9), which is what makes a
 * concurrent revoke win.
 */
import {
  Body,
  Controller,
...
import {
  ...
  type FinalPaymentAttemptHttpView,
...
@ApiExtraModels(FinalPaymentAttemptResponse)
@Controller('public/orders')
export class PublicOrderFinalPaymentAttemptController {
  constructor(
    private readonly links: AuthorizeSecureLink,
    private readonly attempts: InitiateFinalPaymentAttemptUseCase,
  ) {}

  @Post('final-payment/attempts')
  @HttpCode(HttpStatus.CREATED)
  @ApiSuccessCode('FINAL_PAYMENT_ATTEMPT_OPENED', 'Final payment attempt opened.')
  @ApiOperation({
    summary: 'Open one bank-transfer attempt against the final payment a secure link opens',
    description:
      'Records that the customer is about to transfer the remaining balance. The order, the ' +
      'REMAINING obligation, the amount, the currency and the step-up evidence are all ' +
      'resolved by the server from the grant — none of them is accepted from the caller, and ' +
      'the DEPOSIT obligation cannot be reached through this route. Inside one transaction ' +
      'the grant is re-checked under its row lock, the order must be AWAITING_FINAL_PAYMENT ' +
      'and the obligation still PENDING, a recent re-verification of the customer’s own ' +
      'contact is required, and one BANK_TRANSFER attempt is created at PENDING for the ' +
      'obligation’s exact amount. This is **not** a payment: no attempt is settled, no ' +
      'obligation is satisfied, no order becomes READY_FOR_DELIVERY and no reconciliation is ' +
      'written — only an Admin verifying that the money arrived can do any of that. Repeating ' +
      'the call with the same Idempotency-Key replays the same attempt and creates no second ' +
      'one; a deliberate retry sends a new key.',
  })
  @ApiHeader({
    ...
    description:
      ...
      'Keys are scoped to the obligation, so one used on the deposit is free here. ' +
      'Never stored in the clear and never echoed back.',
    schema: { type: 'string' },
  })
  @ApiBody({ type: InitiateFinalPaymentAttemptBody })
  @ApiResponse({
    status: 201,
    description: 'The attempt this call opened, or a replay of an earlier identical one.',
    schema: envelopeSchemaOf(FinalPaymentAttemptResponse),
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiResponse({
    ...
    description:
      ...
      'with no live REMAINING obligation. Identical in status, code, message and shape ' +
      'whether the token is unknown, expired, revoked, superseded, for another target, the ' +
      'order has not been created, or a shipping-fee change superseded the obligation. The ' +
      'deposit is never substituted for it.',
    schema: ERROR_SCHEMA,
  })
  @ApiResponse({
    status: 409,
    description:
      'FINAL_PAYMENT_NOT_PAYABLE when the balance is not currently collectable — the order ' +
      'has not reached AWAITING_FINAL_PAYMENT, is on hold, has moved past it, or the ' +
      'obligation is already SATISFIED, so a settled balance cannot be reopened by a new ' +
      'attempt; DUPLICATE_OPERATION when another initiation with this key is still in ' +
      'flight; IDEMPOTENCY_CONFLICT when this key was already spent on a different ' +
      'initiation.',
    schema: ERROR_SCHEMA,
  })
  @ApiResponse({
  ...
  async initiate(
    ...
    const input = initiateFinalPaymentAttemptSchema.parse(body);
