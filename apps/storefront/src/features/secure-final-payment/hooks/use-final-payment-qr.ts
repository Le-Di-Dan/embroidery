 ...
 *
 * `publicOrderFinalPayment_qr` writes nothing — the PNG is generated on the
 * server from immutable inputs on every request and is never stored, so there is
 * no object, asset or storage key for it. Modelling it as a mutation would
 ...
 * after the customer has left the panel that showed it.
 *
 * ## It is enabled from `payable`, never from the order status
 *
 ...
 * nobody owes. The caller passes `enabled` from the panel selection, which is
 * itself gated on `payable` — so the browser never asks for an image the server
 * is already refusing, and §7's rule that `payable` is the gate holds all the
 ...
 * blob itself never reaches `localStorage` or `sessionStorage` — neither can
 * hold a `Blob` in any case, and the point is that no code exists that tries.
 * Revocation happens *before* replacement rather than after, so a re-fetch
 * cannot leak the previous handle.
 *
 ...
 * the actual way to pay. So its absence degrades to the fallback line and the
 ...
 * frames already print on the page — and carries no token, no account number and
 * no attempt id.
 */
...
import { fetchFinalPaymentQr } from '../api/secure-final-payment.client';
import { FINAL_PAYMENT_QUERY_KEYS } from '../model/final-payment-query-keys';

export interface FinalPaymentQr {
...
export interface FinalPaymentQrOptions {
...
const FILENAME_PREFIX = 'ma-qr-thanh-toan-con-lai';

/** Anything outside this set is dropped, so a surprising order code cannot shape a path. */
const FILENAME_SAFE = /[^A-Za-z0-9-]/g;

export function useFinalPaymentQr({
  enabled,
  orderCode,
  runWithSecret,
}: FinalPaymentQrOptions): FinalPaymentQr {
  const query = useQuery({
    queryKey: FINAL_PAYMENT_QUERY_KEYS.qr(),
    queryFn: ({ signal }) => runWithSecret((secret) => fetchFinalPaymentQr(secret, signal)),
    enabled,
  });
