...
/**
 * Optional transfer evidence for the balance: what has been submitted, and
 * sending one more (`APP9-S01` §11, §12, §19, §20).
 *
 * ## It is bound to the attempt this session opened, and to no other
 *
 * `attemptId` comes from the accepted initiation and from nowhere else — never
 * from user input, never from a URL, never from a stored value. §12 is explicit
 * that an arbitrary attempt id may not be trusted, and the shape of this hook is
 * what enforces it: with no attempt the query is disabled and the mutation
 * rejects before it reaches the wire, so the UI naturally requires initiation
 * first rather than inventing an id to get past the gate.
 *
 * The server is not relying on this. `EvidenceAttemptAuthorizer` locks the
 * attempt row and compares its **order** against the one the grant names, so a
 * foreign or fictional id is an indistinguishable 404 there regardless. This is
 * the client half of the same rule.
 *
 * ## The route is the deposit lane's, and that is deliberate
 *
 * `APP9-B02` widened the authorizer to `['DEPOSIT', 'REMAINING']` rather than
 * publishing a second endpoint, so these calls travel a `deposit`-named path
 * (`FU-APP9-B02-01`). No sentence this hook's panel renders repeats that word.
 *
 ...
 * server refuses because that count was stale the answer is to read again rather
 * than to argue.
 *
 ...
 * the last place to fire a request nobody asked for.
 *
 * ## Uploading is not paying
 *
 * A successful upload refetches the evidence list and **nothing else**. It does
 * not re-read the balance, does not touch the QR, and cannot move the screen to
 * a settled state. The invalidation names exactly one key, because §19 forbids
 * widening it to "everything" because that is easier to write — nothing in the
 * Storefront catalog or custom-request caches has any relationship to an image
 * this customer just sent.
 */
...
import { readTransferEvidence, uploadTransferEvidence } from '../api/secure-final-payment.client';
import { FINAL_PAYMENT_QUERY_KEYS } from '../model/final-payment-query-keys';
import {
...
export function useTransferEvidence({
  ...
  const list = useQuery({
    queryKey: FINAL_PAYMENT_QUERY_KEYS.evidence(attemptId ?? ''),
    queryFn: () => {
