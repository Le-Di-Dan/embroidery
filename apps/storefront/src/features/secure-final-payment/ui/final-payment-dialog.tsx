...
/**
 * The remaining-balance surface's modal frame (`APP9-S01`).
 *
 ...
 * screens import, and the `secure-final-payment` block its approved stylesheet is
...
export function FinalPaymentDialog({ title, onDismiss, children }: FinalPaymentDialogProps) {
  return (
    <ModalFrame
      title={title}
      onDismiss={onDismiss}
      scrimClassName="secure-final-payment__scrim"
      dialogClassName="secure-final-payment__dialog"
      titleClassName="secure-final-payment__dialog-title"
    >
      {children}
    </ModalFrame>
  );
