 ...
 * one overlay on this route inherits it rather than re-deriving it (§38).
 *
 ...
 *
 * The quotation, deposit and remaining-balance dialogs are styled by, and named
 * for, their own features, and none of them exports its frame. A cross-feature
 * import would couple four screens through a presentation module, which
 * `CLAUDE.md` §5 places at the wrong scope, and §50 forbids this checkpoint
 * from editing APP7 or APP9 source to promote one. The *behaviour* below is the
 * shared thing; promoting it to `shared/` now that there is a fourth consumer
 * is recorded as `FU-APP12-S03-03` and is a checkpoint of its own.
 */
const FOCUSABLE =
...
export function OrderDialog({ title, onDismiss, children }: OrderDialogProps) {
  const titleId = useId();
  ...
  return (
    <div className="secure-order__scrim">
      <div
        className="secure-order__dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby={titleId}
        ref={dialogRef}
      >
        <h2 className="secure-order__dialog-title" id={titleId} tabIndex={-1} data-dialog-heading>
          {title}
        </h2>
        {children}
      </div>
