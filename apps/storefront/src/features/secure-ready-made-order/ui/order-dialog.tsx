...
/**
 * The secure Ready-Made order surface's modal frame (`APP12-S03`).
 *
 ...
 * screens import, and the `secure-order` block its approved stylesheet is
...
export function OrderDialog({ title, onDismiss, children }: OrderDialogProps) {
  return (
    <ModalFrame
      title={title}
      onDismiss={onDismiss}
      scrimClassName="secure-order__scrim"
      dialogClassName="secure-order__dialog"
      titleClassName="secure-order__dialog-title"
    >
      {children}
    </ModalFrame>
  );
