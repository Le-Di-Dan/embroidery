...
/**
 * One copy control, for one exact server value (`745:47`, `750:22`, `753:134`).
 *
 ...
 * from the order code, and a client that "tidied" it would produce a transfer the
 * workshop cannot match to an order.
 *
 ...
 * (`753:138`). Success is announced through a polite live region rather than by
 * changing an icon's colour (`753:141`), and the visible short label stays
 * "Sao chép" so the approved layout is unchanged.
 *
...
export function CopyValueButton({ value, label, doneMessage }: CopyValueButtonProps) {
  ...
  return (
    <span className="secure-deposit__copy">
      <button
        type="button"
        className="secure-deposit__copy-button"
        aria-label={label}
        onClick={() => {
          void onCopy();
        }}
      >
        <span className="secure-deposit__copy-symbol" aria-hidden="true">
          ⧉
        </span>
        {COPY.copy.short}
      </button>
      <span className="secure-deposit__visually-hidden" aria-live="polite">
        {feedback}
      </span>
    </span>
  );
