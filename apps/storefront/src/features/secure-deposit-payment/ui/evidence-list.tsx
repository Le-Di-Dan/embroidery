...
import { DepositStatusPill, type PillTone } from './deposit-status-pill';

/**
 * The images already submitted for one attempt (`748:57`, `748:86`, `748:115`,
 * `748:157`, `750:389`).
 *
 ...
 * failed. The panel above carries the approved warning that spells this out; the
 * rows themselves simply never use a payment word.
 */
...
export function EvidenceList({ items }: EvidenceListProps) {
  if (items.length === 0) {
    return <p className="secure-deposit__fine-print">{COPY.evidence.empty}</p>;
  }

  return (
    <ul className="secure-deposit__evidence-list" aria-label={COPY.evidence.listLabel}>
      {items.map((item) => {
        const status = COPY.evidenceStatus[item.assetStatus];
        return (
          <li className="secure-deposit__evidence-item" key={item.evidenceId}>
            <span className="secure-deposit__evidence-glyph" aria-hidden="true">
              ▣
            </span>
            <span className="secure-deposit__evidence-body">
              <DepositStatusPill
                tone={TONES[evidenceToneOf(item.assetStatus)]}
                label={status.label}
              />
              <span className="secure-deposit__evidence-meta">
                {`${formatInstant(item.createdAt)} · ${item.mediaType} · ${formatByteSize(item.byteSize)}`}
              </span>
              <span className="secure-deposit__evidence-note">{status.note}</span>
            </span>
          </li>
        );
      })}
