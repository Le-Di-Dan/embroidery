/**
 * The Storefront Content-Security-Policy (`APP12-H01` §12, hardened by
 ...
 * convenience: React writes element `style` attributes, and the QR and
 * placement surfaces position by inline style. A style nonce cannot cover an
 * attribute — only a `<style>` element — so nonce-ing styles would break the
 * app while claiming a stricter policy. `script-src` grants no `'unsafe-inline'`
 * and no `'unsafe-eval'` in production; the nonce replaces both.
 *
