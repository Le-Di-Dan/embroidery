 ...
 *
 * The **vocabulary** — the event name, the two versions, the association
 * discriminator and the payload shape — is owned by `@embroidery/domain-types`
 * since `APP3-B01N`, because a producer now exists in another application and
 * two independent declarations of one contract drift silently. The names below
 * are this job's local aliases for it, kept so nothing else in the consumer had
 * to change. What stays here is the *parsing*: whether a message is acceptable
 * is the consumer's judgement, and a producer that validated its own output
...
export const ASSET_NORMALIZATION_EVENT_TYPE = ASSET_NORMALIZATION_REQUESTED_EVENT_TYPE;
export const ASSET_NORMALIZATION_PAYLOAD_VERSION = ASSET_NORMALIZATION_EVENT_SCHEMA_VERSION;
...
export const ASSOCIATION_REF_FIELDS = ASSET_NORMALIZATION_ASSOCIATION_REF_FIELDS;

export type AssociationRefKind = AssetNormalizationAssociationKind;
export type AssociationRef = AssetNormalizationAssociationRef;
export type AssetNormalizationPayload = AssetNormalizationRequestedPayload;
