...
- **APP0-T01 — Application integration harness adapter** *(testing foundation; dep: C01)* — **DONE** (adapter at `apps/api/test/support/`; shared `CleanupStack` in `@embroidery/test-utils`; report `../reports/APP0-T01-COMPLETION-REPORT.md`; correction `../reports/APP0-T01-C1-CORRECTION-REPORT.md`).
  ...
  - Correction **APP0-T01-C1** (commit `2f7f274`): the harness was moved out of the production source tree into `apps/api/test/{support,integration}/**` (guarded by `tools/check-api-dist-boundary.mjs`), and a direct `verifySchemaBaseline` proof (adapter-provisioned database → 31 migrations / 78 tables / frozen fingerprint) was added. Final T01 verdict PASS.
- **APP0-DEC-COMPONENT-TEST — Component testing tool decision** *(decision/ADR; separate ADR; IMP-O005a)* — **COMPLETE** (decision `IMP-D024`, resolves `IMP-O005(a)`; report `../reports/APP0-DEC-COMPONENT-TEST-COMPLETION-REPORT.md`).
  - Decided: **Jest** (sole runner, IMP-D016 not reopened) + **`next/jest`** transformer + **`jsdom`** (`jest-environment-jsdom 30.4.1`) + **React Testing Library** (`@testing-library/react 16.3.2` + `@testing-library/dom 10.4.1`) + **`@testing-library/user-event 14.6.1`** + **`@testing-library/jest-dom 7.0.0`** (all MIT). Per-app `next/jest` config + app-root `jest.setup.ts`; shared render helper/provider wrappers/`next/navigation`+`next/image` mocks in a new `@embroidery/frontend-testing` package (test-utils stays backend-neutral); colocated `*.test.tsx` test files, test-support never under `src`; async Server Components covered by E2E (not jsdom); no live network (mock the injected Axios instance / generated operation); snapshots not primary; `v8` coverage with no APP0 threshold. Two finalists spiked against real Next 16.2/React 19.2 in OS-temp (deleted before commit); Vitest and Playwright CT rejected. Decision commit `ce9c8d8`.
- **APP0-T02A — Frontend component and accessibility harness** *(testing foundation; dep: DEC-COMPONENT-TEST, S01B)* — **COMPLETE** (shared package `@embroidery/frontend-testing`; per-app `next/jest` + `jsdom` configs; report `../reports/APP0-T02A-COMPLETION-REPORT.md`).
  - Delivered: the locked Jest + `next/jest` + `jsdom` + React Testing Library stack for admin and storefront. New `@embroidery/frontend-testing` package (render helper, deterministic `QueryClient` factory, `next/navigation` mock, user helper); per-app `jest.config.mjs` (default jsdom) + `jest.setup.ts`; representative jsdom component tests (interaction, `next/link`/`next/image`, mocked `next/navigation`, TanStack Query, public `@embroidery/api-client` import through an injected mocked Axios instance, SCSS side-effect) and test-only fixtures. Existing smoke tests moved to `apps/<app>/test/smoke/**` under the Node environment; all frontend tests live under `apps/<app>/test/**`, never production `src/**`. Static + post-build boundary gates (`tools/check-frontend-{test-boundaries,build-boundary}.mjs`); the static gate joins root `quality`. Windows + Linux (`node:22-bookworm`) test/build parity; no live network; `v8` coverage; production `.next` free of test code. Implementation commit `711b9ea`. Deviation D-1: the framework-agnostic shared package (no Next asset imports) runs its own unit tests on ts-jest + jsdom, as `next/jest` requires an app directory; the apps' component tests use `next/jest` as locked.
- **APP0-DEC-E2E — Browser E2E tool decision** *(decision/ADR; separate ADR; IMP-O005b)* — **COMPLETE** (decision `IMP-D025`, resolves `IMP-O005(b)`; report `../reports/APP0-DEC-E2E-COMPLETION-REPORT.md`; decision commit `88ba26b`).
  - Decided: **Playwright Test** (`@playwright/test 1.61.1`, Apache-2.0) as a **separate browser tier** (Jest stays the sole unit/component runner, IMP-D016 not reopened). Dedicated private package `@embroidery/e2e-testing` (`packages/e2e-testing/`) owns `playwright.config.ts`/`specs`/`support`, reusing the T01/DB7 disposable-DB owners; tiered browser matrix (local/CI Chromium; full/scheduled + Firefox + WebKit); single canonical orchestrator (gateway + apps + API + disposable Postgres); public-routes-only auth for now with a future `storageState`/API-setup path; real-network default; semantic-locator + bounded-CI-retry policy; failure-only screenshot/trace artifacts (none committed); E2E excluded from the fast `pnpm quality`, run in a separate CI tier. Two finalists (Playwright, Cypress) spiked against the real Admin + Storefront in OS-temp (deleted before commit), plus a Linux run in the pinned official image; Cypress and WebdriverIO rejected.
- **APP0-T02B — Browser E2E harness** *(testing foundation; dep: DEC-E2E, S01B, B03)* — **COMPLETE** (package `@embroidery/e2e-testing`; report `../reports/APP0-T02B-COMPLETION-REPORT.md`; implementation commit `a90a9f9`).
  - Delivered: the locked Playwright `1.61.1` tier as a new `@embroidery/e2e-testing` package with one canonical Node orchestrator (`run-e2e.mjs`) that owns the whole lifecycle — ephemeral Postgres + real Nginx gateway via `docker-compose.e2e.yml`, a **disposable** database from the canonical DB7/T01 harness (persistent-DB refusal, 31 migrations, 78 tables, frozen fingerprint), and the API + both Next apps as host processes from built artifacts — and tears everything down in `finally`/on signals. One Storefront smoke, one Admin smoke, and an API-readiness check run **through the real gateway** across Chromium/Firefox/WebKit (host Chromium + pinned `v1.61.1-noble` Linux image; hostnames resolved with no hosts-file edit). Static + post-build boundary gates (`tools/check-e2e-boundaries.mjs`); browser-free `check:e2e` joins `pnpm quality`; `quality:e2e` runs the full matrix. Failure trace/screenshot + success/failure cleanup proven on Windows and Linux; existing suites, OpenAPI and generated client unchanged. Correction (DEC-E2E §2.1): the official image ships browsers, not the npm package — installed separately and version-aligned.
