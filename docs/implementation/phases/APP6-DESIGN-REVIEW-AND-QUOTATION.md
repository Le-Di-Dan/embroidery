...
The gate was **closed at delivery**: `APP6-A01`, `APP6-A02`, `APP6-S01` and
`APP6-S02` may not start until a human reviewer promotes the rows their
checkpoint consumes. The twelve `FIG-APP6-A01-*` rows were promoted to
`APPROVED_FOR_IMPLEMENTATION` under `FIG-APPROVAL-APP6-D01-PO-001`, which is
what opened `APP6-A01`; the rows the remaining three consume are unaffected.
...
| `APP6-A01` | `COMPLETE` | Admin quotation workbench. One screen at `/requests/{requestId}/quotation`, **0 new HTTP operations** (78 → 78), no migration (36). Ships the PO-authorized §4 narrow read-contract unblock: `adminCustomRequest_detail` gains exactly one nullable `quotationId`, resolved from the unique `quotations.custom_request_id` relation and **never** from `custom_requests.current_quotation_id` — that pointer is the customer-current/send authority and is `NULL` until the first send, which is precisely the state an Admin workbench works in, so deriving the locator from it would make an unsent `DRAFT` undiscoverable. The bridge is a new read-only `QUOTATION_LOCATOR_PORT` returning an **id**, not the delivered `QUOTATION_REPOSITORY` whose `findByRequest` would have answered the same question — that repository also carries `send`, `accept`, `reject`, `expire`, `addVersion` and `setCurrentVersion`, and `QuotationReadModule`'s own principle is that what a module can inject is what its routes can eventually do. `@ApiProperty({ type: String, nullable: true })` is explicit so the field does not inherit `FU-APP6-B02-NULLABLE-OBJECT-TYPE-DEBT-01`'s index-signature debt; the artifact delta is one property on one schema with 0 operations added, removed or moved. Exact money is string-only end to end — no `Number`, `parseFloat`, unary `+` or `Math.round` on an amount anywhere in the feature, enforced by a source-shape guard that walks every file and strips comments, and the authoring body carries no derived figure because `APP6-B01`'s schema is `.strict()`. COP offers no `PRODUCT` line and neither branch offers `SHIPPING` or `ADJUSTMENT`, which have their own body fields and would double-count against CST-064. The adjustment-reason rule is mirrored **bidirectionally**, as `computeDraftPricing` enforces it. The send is bodyless, bound to the exact version the dialog was opened for, guarded by a synchronous ref because `disabled` is one render behind a double-click, reports a replay as a replay, and on a stale refusal re-reads and stops with no resend and no substituted version. Loading, empty and failed never collapse: the history query is enabled only once a locator exists. `QUOTATION_ALREADY_EXISTS` is reconciled by re-reading the locator, never by parsing the refusal — proved with an error message carrying a *different* quotation id. The accepted state claims no payment, Order or reserved stock and offers no re-quote (`FU-APP6-B03-REQUOTE-AFTER-ACCEPTANCE-01`). **82 focused Admin tests** plus 6 locator integration, 15 detail integration and 10 contract, with **five mutation proofs** confirming the suites discriminate. All twelve approved `FIG-APP6-A01-*` rows consumed; Figma unmutated and the registry unchanged. **Browser acceptance closed by `APP6-A01-C1`**: all 11 desktop states at 1440 and the narrow 1280 frame verified on the real protected route behind a real staff session, after refreshing two stale dev runtimes — the API image, which predated the bridge, and the `staff-bootstrap` one-shot, which had never published `quotation.validity`, so the first live send correctly refused with a 503. The live locator smoke is the strongest available form of the bridge proof: a request whose `current_quotation_id` is `NULL` while the relation holds an unsent `DRAFT` — found and opened against the live API. The pass found **two real copy defects** that jsdom and 82 green tests could not see: the context rail rendered `{detail.status}` raw, putting `UNDER_REVIEW` in front of an operator on an otherwise Vietnamese screen, and the version-history column borrowed the *request*-status label for a column carrying the *version* status, so one page named two different things identically. A third finding was a test asserting `toHaveTextContent('UNDER_REVIEW')`, which **encoded** the defect rather than catching it — the reason the suite was green and the screen was wrong at the same time. Fixed with a total `presentRequestStatus` mirroring `APP5-A02`'s approved labels and a column label of its own; **85 focused Admin tests**, mutation-proved. `FU-APP6-A01-BROWSER-REVIEW-01` **CLOSED**. Reports [`../reports/APP6-A01-COMPLETION-REPORT.md`](../reports/APP6-A01-COMPLETION-REPORT.md) and [`../reports/APP6-A01-C1-COMPLETION-REPORT.md`](../reports/APP6-A01-C1-COMPLETION-REPORT.md) |
| `APP6-A02` | `COMPLETE` | Admin design-case workbench. One screen at `/requests/{requestId}/design`, **1 new HTTP operation** (78 → 79), no migration (36). Ships the PO-authorized §5 unblock: `adminCustomRequestDesignVersion_detail`, the exact-version read `APP6-B08` explicitly routed forward when it left the document out of version history. It is a **read by construction**: a new four-SELECT `DESIGN_VERSION_DETAIL_PORT` rather than `DESIGN_CASE_REPOSITORY` — that repository carries five writes and additionally requires `PLACEMENT_HIERARCHY_PORT`/`CatalogModule`, so the module boundary is what makes authoring, sending, deciding, transitioning and emitting unreachable rather than merely unused, proved by the send and create routes answering **404 from the router** inside the booted injector. `FU-APP6-B09-CASE-REPO-SIZE-01` stays closed: the 399-line AGG-10 adapter is untouched. The version lookup is **case-bound in the query itself** (`findVersion(caseId, versionId)`), so a foreign version is unreachable rather than refused and answers identically to a missing one — no `403` exists on the operation. The document is the single generated P01 component by `$ref`, returned verbatim for both v1 Catalog and v2 COP; `documentHash` is B09's stored value, null when unsent, never recomputed. Feedback is `design_reviews.feedback`, read by an adapter that selects three columns and never retrieves `customer_id`/`grant_id`/`step_up_challenge_id`. The approval projection reads **snapshot truth**, proved by renaming the live product *after* approval and asserting the frozen name survives; contacts are masked once in the application layer and `ApprovalEvidenceView` has no field an unmasked value could occupy; `reverified` is the boolean the committed step-up evidence reduces to. The agreement **version integer is deliberately not published** — it lives in AGG-21 and the only port reaching it also carries `publishVersion`/`withdrawVersion`, so the content hash `GRD-008` binds to identifies the accepted version instead. The operation id minted as `…DesignVersionDetail_detail` and was brought into the accepted family through `CONTROLLER_DOMAIN_KEYS`, the seam that exists for exactly this split; B08's and B09's three ids are untouched. Frontend: append-only history, a revision copying the **exact predecessor's** document rather than "latest", Catalog placement as server authority with `{ document }` the whole body, bodyless send guarded by an in-flight ref, `REVIEW_ALREADY_ACTIVE` as its own re-read-and-stop outcome, and a truthful gate that **links to** `APP6-B06` rather than duplicating its transition. "Mở trình số hoá" is A02-local composition over the public `design-document`/`design-engine` APIs — `EditorStage` is coupled to `ResolvedTemplateScope`, which COP has no equivalent for — so there is no deep import, no second schema, no second geometry authority and no second rendering engine. **199 focused tests** (16 contract, 22 integration, 68 Admin, 93 shared-status compatibility) with **five mutation proofs**, two of which exposed *vacuous* tests: the duplicate-send guard passed with the guard deleted until the clicks were dispatched as raw `MouseEvent`s (both `userEvent` and `fireEvent` flush React, so `disabled` was doing the work), and a deep-import guard was scanning `undefined` after a field rename — caught by ESLint, not by running. **Browser acceptance closed in the first attempt**: all 13 approved rows consumed, 12 verified live at 1440 and the narrow 1280 frame verified, after repairing two pre-existing environment faults — a stale Admin dev container and a **dev database two migrations behind**, whose missing `design_versions.customer_owned_product_id` was making B08's *delivered* list endpoint 500. The pass found **one real defect**: the controller returned `data: { version: … }` while the published schema declared `data` **as** the version, so the generated client read `undefined` and the revision panel rendered no feedback, no preview and no hash — green in every suite, because the contract spec only inspects the document and the integration assertions read `.version.*`. Fixed by unwrapping; the OpenAPI artifact is unchanged, confirming the contract always said the flat shape, and a shape-pinning test now fails on the regression. `FU-APP6-A01-C1-REQUEST-STATUS-COPY-DUPLICATION-01` **CLOSED** by extracting `shared/presentation/request-status.ts` and deleting the duplicate blocks from both copy catalogs. `698:3` (loading) is asserted in the component suite and renders, but could not be held open for a recorded browser observation — `FU-APP6-A02-LOADING-FRAME-BROWSER-OBSERVATION-01`. Report [`../reports/APP6-A02-COMPLETION-REPORT.md`](../reports/APP6-A02-COMPLETION-REPORT.md) |
| `APP6-S01` | `COMPLETE` | Customer secure quotation screen. One Storefront screen at `/truy-cap/bao-gia`, **0 new HTTP operations** (79 → 79), no migration (36), OpenAPI artifact untouched. The APP4 secure-link shell with the APP6 quotation inside it: capture `#t=` → `history.replaceState` → clean URL → token in a POST **body**, verified live at the moment the request left. The one change the checkpoint needed is an opt-in `retainCredentialAfterSuccess` on `useSecureLinkBootstrap` — every earlier landing is read-only, so the credential died with its single call; here the same grant authorises the customer's later decision, the fragment is already gone and nothing may persist it, so it stays in the **same ephemeral ref** and nowhere else. Off by default, and APP4-S02/APP5-S02 re-run byte-identical. Step-up reuse is **strategy B**: the verification purpose was parameterised at the controller and transport seam of `contact-verification`, so `/xac-minh-lien-he` and the quote step-up run one implementation, one code format, one cooldown source — and the step-up opens **inside** the route behind `701:3`'s scrim, with `/xac-minh-lien-he`, `useRouter`, `redirect(` and `router.push` statically proved absent. A completed step-up is **evidence, never consent**: it triggers the mandatory B04 re-read, and the same version returns the customer to a confirmation they must press again. Every decision names the exact `versionId` B04 returned, captured once and never re-derived; the decision path has no notion of latest. Money is server strings end to end — no `Number`/`parseFloat`/`parseInt`/`toFixed`/`Intl.NumberFormat`, and exactly **one** `Math.ceil` in the whole feature, on a date difference — all enforced by a static boundary test rather than review. `FU-APP6-B04-CUSTOMER-ADJUSTMENT-EXPLANATION-01` **CLOSED**: the amount is shown truthfully under a neutral label and `adjustmentReason` does not exist as a string in the feature, proved live against a seeded version carrying a conspicuous internal note. Expired renders **no acceptance control at all**, not a disabled one; a 404 and a missing fragment are one indistinguishable state with no diagnostic second request; same-tick double activation sends one request, guarded by a synchronous ref because `isPending` is a render behind. **89 focused tests** (5 suites: model, screen, step-up, secrecy sweep, static boundary) plus **137 APP4/APP5 compatibility**. Browser acceptance PASS at 1440 and 390 (no horizontal overflow, 52px touch targets), **0 browser-found defects**, after two pre-existing environment repairs (stale container; APP4 peppers unset in `.env`, provisioned by the operator — no secret read, echoed or committed). Three frames could not be observed live because APP4's only notification channel is memory-only by design, so no verification code exists to read in dev — `FU-APP6-S01-STEPUP-BROWSER-OBSERVATION-01`. Report [`../reports/APP6-S01-COMPLETION-REPORT.md`](../reports/APP6-S01-COMPLETION-REPORT.md) |
| `APP6-S02` | `COMPLETE` | Customer secure design review screen. One Storefront screen at `/truy-cap/duyet-thiet-ke`, **0 new HTTP operations** (79 → 79), no migration (36), OpenAPI artifact untouched. The APP4 secure-link shell with the APP6 review inside it, reusing `APP6-S01`'s accepted credential architecture **without changing one line of it**: capture `#t=` → `history.replaceState` → clean URL → token in a POST body, `retainCredentialAfterSuccess` already opt-in, so no APP4/APP5 compatibility suite was impacted. Renderer reuse is **strategy C**, chosen because A is impossible (`design-studio` exports nothing renderer-shaped; its only painter is an interactive `role="button"` control) and B is a larger refactor (`test/boundary/design-studio-source.test.ts` is a closed APP3 acceptance control that enumerates the feature directory and rules "exactly one production renderer", one `<svg>`, and a four-file watermark ownership — moving files out changes what it measures). C is genuinely smaller here: no selection, no gesture, no per-frame document replacement, no structural sharing, no stroke-aware bounds. Every guardrail is structural and statically pinned — one validator (`validateDesignDocumentStructure`), one geometry authority (`@embroidery/design-engine`, with `Math.cos/sin/atan2/PI` and matrix composition banned in-feature), native SVG only (canvas/konva/fabric/pixi banned, **exactly one** `<svg>` asserted), and the document returned untouched. Catalog v1 and COP v2 both render, the COP branch without the adapter ever reading `productSideId`. The watermark is mounted **unconditionally** — no prop, flag or ternary decides it — and `download`, `createObjectURL`, `Blob(`, `toDataURL`, `navigator.clipboard`, `XMLSerializer`, `JSON.stringify` and all three browser stores are statically absent from the feature. Agreement consent is keyed by the **signature of the set it was given for**, so a changed `contentHash`, id, order or membership yields empty consent **by construction — there is no reset to delete**; `PAYMENT_POLICY`, `RETURN_POLICY` and `DESIGN_APPROVAL_TERMS` appear nowhere in the source, and an invented type renders correctly. Approval binds a decision snapshot captured **once**; a completed step-up is evidence, never consent, and the post-step-up re-read compares version **and** stored hash together before comparing terms — a terms change is never reported as a design mismatch. Every reconciliation is exactly **one** re-read; `DUPLICATE_OPERATION` deliberately triggers **zero**. **150 focused tests** (5 suites: model, screen, decisions, secrecy sweep, static boundary) plus **44 api-client**, and all **six** mutation proofs discriminate — M2 needed a *static* pin because the exactness is enforced twice over and is therefore behaviourally undetectable. Browser acceptance PASS at 1440 and 390 against the real stack (no horizontal overflow, 52px touch targets, zero token leaks across eight surfaces at four points, revision committed live with the database confirming the request did **not** move). **Two defects the browser caught that jsdom could not**: raw ISO instants rendered to the customer, and a native `required` that swallowed the submit event and would have accepted whitespace-only feedback. Four frames could not be observed live because dev cannot complete a step-up by design — `FU-APP6-S01-STEPUP-BROWSER-OBSERVATION-01`, carried. One authority discrepancy reported rather than resolved silently: `FU-APP6-S02-WATERMARK-DRAWN-VS-DELIVERED-01`. Report [`../reports/APP6-S02-COMPLETION-REPORT.md`](../reports/APP6-S02-COMPLETION-REPORT.md) |
| `APP6-E01` | `COMPLETE` | Focused cross-layer acceptance — **PASS**, 0 blocking defects. **0 new HTTP operations** (79 → 79), no migration (36), OpenAPI artifact read and never regenerated (72 paths / 79 operations / 167 schemas), **no production runtime change**. Six serial cases in one composed injector carrying every delivered APP6 module at once, over real HTTP through real guards, against a disposable PostgreSQL with every migration: `E01-01` Catalog and `E01-02` COP both reach `APPROVED` through owning operations only; `E01-03` quotation stale/expiry/immutability; `E01-04` design race/revision/first-decision-wins/immutability; `E01-05` grant reuse, non-enumeration and the step-up matrix; `E01-06` the customer-safe contract surface and the APP7 boundary, statically over the committed artifact and again at runtime. Policy values are the delivered ones — the three `staff-bootstrap` publishers run against the committed seed datasets, so the 7-day validity window, the 40/60 deposit split, the `PAYMENT_POLICY` + `RETURN_POLICY` required set and the agreement text are production's. The harness seeds only prerequisite state up to `UNDER_REVIEW` and holds no route or statement that can write a status: both journeys record the ledger `UNDER_REVIEW → QUOTED → QUOTE_ACCEPTED → DIGITIZING → DESIGN_REVIEW → APPROVED` with four `SYSTEM` projections and exactly one `ADMIN`-commanded move (`DIGITIZING`, `GRD-005`). `GRD-003` executes for real throughout — the fixture commits a `VERIFIED` `STEP_UP` challenge row, it does not bypass a guard, and no plaintext verification code exists in the process. Watermark runtime authority recorded as `DELIVERED_APP3_S09` and proved cross-layer by behavioural parity between the Studio and the review models. `CMD-TEST-APP6-E01`, `CMD-TEST-APP6-E01-WATERMARK`. **`APP6-E01-C1` (evidence correction, docs-only, no rerun)** withdrew E01's false conclusion that no delivered operation creates a `design_cases` row: the delivered owner is **APP5-B01 / `TR-LC11-01` `request.submit`**, which creates the header inside the idempotent W1 submission transaction (`SubmitCustomRequestUseCase` → `DesignCaseRepository.createForRequest`; APP5-B01 proved 1 case on first submission, on replay and under the CC-18 concurrent duplicate). E01 seeds the row only because it starts at the APP6 prerequisite boundary with the request already `UNDER_REVIEW` and therefore does not re-execute APP5 submission. `FU-APP6-E01-DESIGN-CASE-ORIGIN-01` = `CLOSED_FALSE_POSITIVE`; `APP6-B08`'s existing-case semantics and the E01 `PASS` verdict are unchanged. |
| `APP6-X01` | `COMPLETE` | **Phase closure — APP6 is CLOSED.** Documentation and evidence reconciliation only: **no production runtime change**, no schema/migration change (36), no OpenAPI/client generation, no Figma mutation, no implementation suite rerun. Recomputed from repository truth rather than assumed: the committed artifact carries **72 paths / 79 operations / 167 schemas** (blob `80ad1b25…`), and APP6 ownership was established by `operationId` set difference against the APP5 frozen baseline at `6b09c04` (58/63/132) — **+14 paths, +16 operations, +35 schemas, +1 migration, 0 operations removed**. The **16** APP6 operations reconcile to the roadmap distribution line for line, including the two zero-operation checkpoints (`B06`, `A01`). Migration `0036_add_app6_cop_design_context.sql`; 56 `APP_06` Figma rows, **all** `APPROVED_FOR_IMPLEMENTATION` under `FIG-APPROVAL-APP6-D01-PO-001`. **22 APP6-owned follow-ups** were swept from repository truth — 7 more than the closure brief listed — plus 2 inherited ids; **10 resolved, 12 nonblocking, 0 blocking**. Exactly one was ever genuinely blocking (`FU-APP6-A01-BROWSER-REVIEW-01`), closed by `APP6-A01-C1` before closure. Two items were closed by authority routing: the AGG-14 lock-order property, promoted into the closure matrix so the next writer does not learn it by deadlocking, and the watermark — **`APP6_WATERMARK_RUNTIME_AUTHORITY = DELIVERED_APP3_S09`** (−30°, 5 × 7 percentage grid), verified by reading both models at HEAD and found identical, with the `18°` / `160 × 130` figures in `APP6-D01` and the Figma registry annotated as a **historical measurement of the reproduction** rather than rewritten away. `FU-ADMIN-SHARED-DIALOG-01` is recorded honestly: `APP4-X01` routed it here on a phase-title assumption, APP6 did **not** consolidate it, and it is re-routed rather than quietly closed. Matrix [`../reports/APP6-CLOSURE-MATRIX.md`](../reports/APP6-CLOSURE-MATRIX.md); report [`../reports/APP6-X01-COMPLETION-REPORT.md`](../reports/APP6-X01-COMPLETION-REPORT.md) |
...
A01 = COMPLETE (BROWSER ACCEPTANCE CLOSED BY APP6-A01-C1)
A02/S01/S02 = NOT STARTED
NEW HTTP OPERATIONS = 0 (78 → 78)
A01_DRAFT_QUOTATION_DISCOVERY = NARROW_CONTRACT_UNBLOCK
DATABASE MIGRATION = NONE (36)
FOCUSED TESTS = 85 ADMIN + 31 API
BROWSER 1440 = PASS (11/11)   BROWSER 1280 = PASS
...
```text
APP6-A02 = COMPLETE
SCREEN = ADMIN DESIGN-CASE WORKBENCH   ROUTE = /requests/{requestId}/design
A02 APPROVED FRAMES = 13 / CONSUMED (12 VERIFIED LIVE; 698:3 PARTIAL)
NEW HTTP OPERATIONS = 1 (78 → 79)     DATABASE MIGRATION = NONE (36)
A02_EXACT_VERSION_DETAIL = PO_AUTHORIZED_NARROW_READ_UNBLOCK
READ MODULE = NO CASE REPOSITORY / NO SNAPSHOT REPOSITORY / NO TX / NO OUTBOX
FOREIGN VERSION = INDISTINGUISHABLE FROM MISSING (404, NO 403 ON THE OPERATION)
EXACT VERSION DOCUMENT = CONCRETE P01 ($ref, v1 + v2 VERBATIM)
REVISION FEEDBACK = PERSISTED DECISION RECORD
APPROVAL SNAPSHOT READ = FROZEN EVIDENCE (LIVE RENAME PROOF)
MỞ TRÌNH SỐ HOÁ = FORMAL DESIGNDOCUMENT AUTHORING / SECOND ENGINE ABSENT
SEND = B09 BODYLESS / DUPLICATE BLOCKED / REVIEW_ALREADY_ACTIVE PROVED LIVE
REQUEST STATUS PRESENTATION = ADMIN-SHARED / THIRD MAP ABSENT
FOCUSED TESTS = 199 (16 CONTRACT + 22 INTEGRATION + 68 ADMIN + 93 COMPATIBILITY)
MUTATION PROOFS = 5 (TWO EXPOSED VACUOUS TESTS)
BROWSER 1440 = PASS (698:3 PARTIAL)   BROWSER 1280 = PASS
BROWSER-FOUND DEFECTS = 1 FIXED (ENVELOPE SHAPE DIVERGED FROM PUBLISHED CONTRACT)
DEV ENVIRONMENT REPAIRS = 2 PRE-EXISTING (STALE ADMIN CONTAINER, DB 34/36 → 36/36)
FU-APP6-A01-C1-REQUEST-STATUS-COPY-DUPLICATION-01 = CLOSED_BY_APP6_A02
FU-APP6-A02-LOADING-FRAME-BROWSER-OBSERVATION-01 = OPENED (NONBLOCKING)
...
### 11.1a Final phase status

```text
APP6-R00  = COMPLETE
APP6-G01  = COMPLETE          (APP6-G01-C1 incorporated)
APP6-DB01 = COMPLETE
APP6-D01  = COMPLETE          (PRODUCT_OWNER_APPROVED, 56 rows)
APP6-B01  = COMPLETE
APP6-B02  = COMPLETE
APP6-B03  = COMPLETE
APP6-B04  = COMPLETE
...
APP6-B10  = COMPLETE
APP6-B11  = COMPLETE
APP6-A01  = COMPLETE          (APP6-A01-C1 incorporated)
APP6-A02  = COMPLETE
APP6-S01  = COMPLETE
APP6-S02  = COMPLETE
APP6-E01  = COMPLETE — PASS   (APP6-E01-C1 incorporated)
APP6-X01  = COMPLETE

APP6                             = CLOSED
APP6 VERDICT                     = PASS_WITH_FOLLOW_UPS
BLOCKING FOLLOW-UPS              = 0
APP6 CROSS-LAYER ACCEPTANCE      = PASS
OPENAPI BASELINE                 = 72 paths / 79 operations / 167 schemas
APP6-OWNED OPERATIONS            = 16
MIGRATIONS                       = 36 (APP6-owned: 0036_add_app6_cop_design_context)
FIGMA APP_06 ROWS                = 56 / 56 APPROVED_FOR_IMPLEMENTATION
APP6_WATERMARK_RUNTIME_AUTHORITY = DELIVERED_APP3_S09
FU-APP6-E01-DESIGN-CASE-ORIGIN-01 = CLOSED_FALSE_POSITIVE (owner APP5-B01 / TR-LC11-01)
FU-APP6-S02-WATERMARK-DRAWN-VS-DELIVERED-01 = CLOSED_BY_AUTHORITY_ROUTING
FU-APP6-B05-AGG14-LOCK-ORDER-01  = CLOSED_BY_AUTHORITY_ROUTING

NEXT PHASE = APP7 — Deposit Payment and Order Creation (NOT_STARTED)
```

Canonical closure artifacts:
[`../reports/APP6-X01-COMPLETION-REPORT.md`](../reports/APP6-X01-COMPLETION-REPORT.md) ·
[`../reports/APP6-CLOSURE-MATRIX.md`](../reports/APP6-CLOSURE-MATRIX.md).

Historical checkpoint rows above are retained unchanged; the closure matrix is
the current world where it and older planning prose disagree.
