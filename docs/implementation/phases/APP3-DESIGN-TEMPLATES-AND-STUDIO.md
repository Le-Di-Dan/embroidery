...
FU-APP3-TEMPLATE-SOURCE-ASSET-INTAKE-01 = OPEN — OWNER_NOT_YET_ASSIGNED
FU-APP3-TEMPLATE-SCOPE-EDIT-01 = OPEN — OWNER_NOT_YET_ASSIGNED — scope is settable only at creation (CreateDesignTemplateBody) and APP3-A02 deliberately narrowed the create dialog to unscoped drafts, so a Template created through the Admin UI can never become authorable
