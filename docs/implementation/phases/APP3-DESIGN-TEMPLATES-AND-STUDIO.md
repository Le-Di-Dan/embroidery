...
FU-APP3-S10-RETRY-EXTRA-ATTEMPT-01 = OPEN — NONBLOCKING — a failing streak that begins with a customer edit performs one extra save attempt one debounce after the first failure, before the locked [5000, 15000] ladder runs from the second. Measured in the browser, bounded, stopping, and reconciled like every other attempt; not reproduced by the fake-clock unit suites. Needs a controlled reproduction that identifies which scheduler armed it before anything is changed
FU-APP3-STUDIO-TOOL-RAIL-01 = STILL_OPEN_AFTER_APP3-S10 — S10 relocated the .studio-stage__topbar wrapper so the save chip has one region at every tier. It did not move the APP3-S05 text and APP3-S06 image tools into the persistent rail, which is what this follow-up is about, so it is not closed here
APP3-S11 = BLOCKED_BY_APP3-S10_REVIEW_ACCEPTANCE_FOR_PRACTICAL_SEQUENCE — a shared-frontend sequencing guard, not a change to S11's canonical hard dependencies S03 + S07
