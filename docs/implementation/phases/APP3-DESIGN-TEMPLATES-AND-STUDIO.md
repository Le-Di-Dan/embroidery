...
APP3-S11 FOUNDATION_NOTE = P01_AND_P02_PACKAGES_EXIST — NOT_CHECKPOINT_READINESS — the readiness line this note qualified now lives with the delivered APP3-S11 record below; two status lines for one checkpoint is two answers to the same question
...
FU-APP3-TEMPLATE-SOURCE-ASSET-INTAKE-01 = DEFERRED_LATER_APP3 — production Template image intake still has no owner. APP3-E01 fixtures source Template artwork bytes directly, which is a harness capability and not an intake path, so this is deferred rather than closed
...
APP3-S05-MI01 CLASS = FINAL_OPERATOR_INTERVENTION — not a second ordinary correction; the S05 correction budget was consumed by APP3-S05-C1 and no APP3-S05-C2 exists. One goal only: place the 1024 text-inspector drawer trigger in the actual Studio topbar
APP3-S05-MI01 RESIDUAL = TRIGGER_PLACEMENT_IS_PART_OF_THE_APPROVED_COMPOSITION — APP3-S05-C1 knowingly placed the tablet drawer trigger in the APP3-S07 control strip below the stage, reasoning that the strip was the Studio's only persistent control region and that the authority was about the toggle living outside the drawer. Human review rejected that reading: FIG-STUDIO-EDITING-TABLET-1024 (618:140) draws the toggle in a topbar, "topbar" means a control region ABOVE the stage, and functional equivalence does not substitute for an approved placement
APP3-S05-MI01 TOPBAR = FIRST_CHILD_OF_THE_STAGE_FRAME_ABOVE_THE_STAGE — a minimal Studio topbar host rendered before the viewport, carrying the S05 drawer trigger and nothing else. No save chip (S10), no layers trigger or content (S04), no upload (S06), no undo/redo (S08), no watermark (S09) and no mobile control (S11) was prebuilt to make it look full, and no APP3-S07 control was moved into it
APP3-S05-MI01 S07_STRIP = UNCHANGED — zoom, zoom value, fit and the safe-area toggle stay exactly where APP3-S07 put them, below the stage, with exactly the controls S07 gave them. The gate asserts the strip never acquires the text trigger
APP3-S05-MI01 ORDER = DOM_EQUALS_VISUAL_EQUALS_KEYBOARD — the topbar is a real first child, not a later child reordered with CSS. No `order` places the control above the stage while leaving it late in the tab sequence
APP3-S05-MI01 GEOMETRY = SVG_UNCHANGED — the topbar is an in-flow row above the stage and moves the stage's page offset only; it mutates no SVG viewBox, no document canvas and no APP3-P02 geometry, and the drawer stays out of flow so opening it still changes no in-flow box
APP3-S05-MI01 API_DELTA = 0
APP3-S05-MI01 BACKEND_CHANGE = NONE — Storefront only; OpenAPI 35/40/83, 34 migrations, 30 root scripts, generated api-client source, API runtime, worker and database all unchanged, and no dependency added
APP3-S05-MI01 MIGRATION = NONE
APP3-S05-MI01 FIGMA = READ_ONLY — no Figma node was created, moved or mutated, and no registry row changed status
APP3-B06C = COMPLETE — REVIEW_ACCEPTED
APP3-B06C CLASS = REPLAN_CHILD_COMPLETING_APP3-B06 — the *granted delivery* half the APP3-B06 row always carried and APP3-G08 never reassigned. Not a correction of APP3-B06B and not an APP3-B06B-C2
APP3-B06C DELIVERY_CLASS = IMP-D044 PO-06 CLASS 3 — the third and last delivery class; every one of the three is one operation on one contextual address, and none of them is a generic asset API
APP3-B06C OPERATIONS = publicDesignSessionAsset_get
APP3-B06C ROUTE = GET /api/public/design-sessions/{sessionId}/assets/{assetId}/editor-preview — the semantic the pre-implementation audit named, with {sessionId} taken from the delivered Session routes
APP3-B06C CONTROLLER_SPLIT = PublicDesignSessionAssetPreviewController — a second class in the publicDesignSessionAsset domain, mapped through CONTROLLER_DOMAIN_KEYS so the split reissues no accepted operation id (APP3-B04A). Upload keeps publicDesignSessionAsset_create
APP3-B06C AUTHORIZATION = SESSION_ID_PLUS_PER_SESSION_CREDENTIAL — verified on every request through the accepted APP3-B06A primitives; the session id used downstream is the one the credential proved, never the path parameter
APP3-B06C READ_GUARD = DesignSessionReadGuard — composed from the B06A primitives, not the mutation guard. It drops exactly two mutation-specific rules: the PO-05 exact-Origin requirement, which no same-origin <img> can satisfy, and the PO-07 mutation limit, which a preview must not spend. An explicit Sec-Fetch-Site: cross-site is still refused before a cookie is read
APP3-B06C READ_RATE_LIMIT = NOT_INVENTED — IMP-D043 PO-07 defines creation, creation-burst, mutation and authorization-failure and no read limit. None was invented here; the authorization-failure budget still bounds unauthorized probing, and an authorized session probing asset ids is bounded by UUIDv4 unguessability and by every miss being indistinguishable. Recorded as FU-APP3-B06C-READ-RATE-LIMIT-01
APP3-B06C OWNERSHIP = DESIGN_SESSION_ASSET — the association is the first relation of the descriptor statement, so a foreign asset is unreachable rather than filtered; Template lineage, B05A permission and asset existence authorize nothing
APP3-B06C LANE = CUSTOMER_UPLOAD + CUSTOMER_PRIVATE + ACCEPTED — the parent's inspection verdict is asserted independently of the derivative, because a derivative can exist while the asset is still INSPECTING
APP3-B06C DERIVATIVE = READY UNWATERMARKED NORMALIZED + APP3-DB01 QUARTET
APP3-B06C DELIVERABLE_MEDIA = image/webp — narrower than the B06B intake allowlist on purpose: JPEG, PNG and WebP are accepted *uploads*, and APP3-W01A writes one normalized output. SVG is unproducible and undeliverable
APP3-B06C ORDERING = DESCRIPTOR_BEFORE_STORAGE — an unauthorized or ineligible probe makes zero object-storage calls, so provider load and response timing are not an existence oracle
APP3-B06C MISS_TAXONOMY = ONE_404 — unknown asset, foreign session, missing association, wrong lane, tombstone, INSPECTING, REJECTED, and an absent, unready, watermarked or incompletely described derivative are indistinguishable. Session credential failure stays the accepted 401/429 and is not collapsed into it
APP3-B06C PROVIDER_CONTRADICTION = 503 — a fully authorized descriptor whose object is missing, unreadable or size-mismatched is unavailable, never a privacy 404, and the persisted metadata is never repaired from provider state
APP3-B06C HEADERS = no-store + nosniff + inline + Content-Length — no filename, ETag, Last-Modified, range or cacheable directive
APP3-B06C WRITES = NONE
APP3-B06C AUDIT = NONE
APP3-B06C EVENTS = NONE
APP3-B06C MIGRATION = NONE
APP3-B06C COOKIE = NEVER_ISSUED_OR_ROTATED — a successful read issues no secret, rotates no cookie, extends no expiry and advances no revision; the guard may only clear a credential that is already dead, under the accepted APP3-B06A rules
APP3-B06C DOCUMENT_MEMBERSHIP = NOT_REQUIRED — deliberately unlike APP3-B05A. A published Template Version is frozen, so its document is the grant; a Session document is a live draft, and requiring membership would make the APP3-S06 upload → preview → place workflow circular
APP3-B06C SURFACE = 36 paths / 41 operations / 83 schemas — measured from the generated artifact; schemas are unchanged because the response is binary and the request has no body
APP3-B06C FRONTEND = NONE — APP3-S06 is the consumer and is not implemented here
APP3-S06 = COMPLETE — REVIEW_ACCEPTED
APP3-S06 CLASS = FRONTEND_CAPABILITY_WITH_AUTHORIZED_ENABLEMENT_REPAIR — the Studio image capability, plus the two cross-layer seams the accepted design directly required and that S06 was authorized to repair inside itself rather than behind a new checkpoint
APP3-S06 FIGMA = 4 ROWS APPROVED — FIG-STUDIO-IMAGE-DESKTOP-UPLOADING 608:343, -NORMALIZING 608:393, -READY 608:441, -FAILED 608:503, section 10 (596:16). No fifth frame, no Studio blanket approval, and FIG-STUDIO-MOBILE-IMAGESHEET 610:465 stays REVIEW_REQUIRED because it is APP3-S11's
APP3-S06 FIGMA_MUTATION = NONE — no node created, moved or edited; only registry status changed
APP3-S06 RESPONSIVE = 1440 EXACT / 1024 SHARED_DRAWER / 390 NO_EDITING — at 1024 the image controls are a section of the one accepted 618:140 drawer behind the one APP3-S05-MI01 topbar trigger, never a second drawer; at 390 nothing that edits is rendered at all, because mobile image tooling is APP3-S11's
APP3-S06 REPAIR_A = SESSION_INSPECTION_LANE — apps/worker asset-inspection accepted only CATALOG_MEDIA + PRODUCTION_SENSITIVE, so every CUSTOMER_UPLOAD inspection job raised a contradiction, dead-lettered, and left the asset in INSPECTING permanently. The pipeline is parameterized by a lane resolved from the locked asset row; the catalog lane is unchanged in every respect
APP3-S06 REPAIR_A_OUTPUT = NO_DERIVATIVE — the Session lane writes none. THUMBNAIL and CATALOG_PREVIEW of a private customer upload would be catalogue media nothing serves, and the editor-safe NORMALIZED output is APP3-W01A's, produced from the event APP3-B06B already emits. No second producer, no second event, no second queue, no scheduler and no sleeping inside an attempt
APP3-S06 REPAIR_A_DECODE = FULL_DECODE_ON_A_LANE_WITH_NO_DERIVATIVE — a finding, not a design: the catalog lane's pixel-level decode was always a *side effect* of generating its outputs, and readSourceMetadata parses only the header. A lane that writes nothing therefore accepted a truncated PNG. verifyFullDecode makes the decode explicit, derived from lane.derivatives.length so no future lane can inherit the weaker check
APP3-S06 REPAIR_A_CONVERGENCE = W01C_UNCHANGED — inspection reaching ACCEPTED is exactly the state APP3-W01C already retries until it sees; the same normalization work converges with no change to it
APP3-S06 REPAIR_B = SESSION_ASSET_STATUS — exactly one narrow contextual read, because APP3-B06C's 404 is deliberately not a status protocol and cannot say whether an upload is processing, rejected or never this Session's
APP3-S06 OPERATIONS = publicDesignSessionAsset_status
APP3-S06 ROUTE = GET /api/public/design-sessions/{sessionId}/assets/{assetId}/status
APP3-S06 CONTROLLER_SPLIT = PublicDesignSessionAssetStatusController — a third class in the publicDesignSessionAsset domain, mapped through CONTROLLER_DOMAIN_KEYS so the split reissues no accepted operation id (APP3-B04A)
APP3-S06 STATUS_STATES = PROCESSING | READY | REJECTED — three, and PROCESSING deliberately does not distinguish inspection from normalization: the Studio shows one state for both and naming the stage would publish the worker pipeline's shape to an anonymous caller. READY is the only state carrying media fields, and they are exactly derivativeId + the APP3-DB01 quartet
APP3-S06 STATUS_AUTHORIZATION = UPLOAD_ASSOCIATION_ONLY — narrower than delivery on purpose: a status projection answers "how is my upload progressing", and cloned Template media has no progression to report because APP3-B07 proved it READY before the Session existed
APP3-S06 STATUS_DISCLOSURE = NONE — no storage key, bucket, provider, checksum, filename, inspection detail, rejection reason, worker error, job id, lease or retry count
APP3-S06 REPAIR_C = CLONED_SESSION_MEDIA_GRANT — APP3-B07's cloneFromTemplate writes no design_session_assets row, so APP3-B06C refused the artwork a CLONE_TEMPLATE Session was created with. The delivery grant is now the accepted APP3-B08 pair: this Session's own upload, OR an image its own persisted document already places
APP3-S06 REPAIR_C_NARROWING = DOCUMENT_BRANCH_IS_TEMPLATE_SOURCE_ONLY — the document branch admits TEMPLATE_SOURCE and nothing else, so a document reference can never reach another customer's CUSTOMER_UPLOAD. APP3-B08's allowlist already prevents such a reference being written; this makes the leak unrepresentable rather than prevented by another module's continued behaviour
APP3-S06 REPAIR_C_LINEAGE = PROVENANCE_NOT_PERMISSION — no branch consults a Template id, slug, version or publication state, so an authorized Session keeps rendering its own accepted document after the Template is unpublished, and knowing a Template grants nothing
APP3-S06 READ_RATE_LIMIT = IMP-D043 PO-07 BOOTSTRAP_RESUME_READ 60_PER_MINUTE_PER_EPHEMERAL_NETWORK_KEY — charged before authorization, so it bounds probing rather than only successful reads. It **is** a locked PO-07 control, ruled at APP3-G03 and carried in the security document's limits table since 2026-08-04; APP3-B06C read PO-07 as defining only four controls, disclosed the absence as FU-APP3-B06C-READ-RATE-LIMIT-01 and declined to invent a number — the right instinct on a wrong premise. What was missing was the implementation, not the decision. Separate dimension from both the mutation counter and the authorization-failure budget
APP3-S06 P01_ELEMENT = ASSET_ID + DERIVATIVE_ID + INTRINSIC_DIMENSIONS — built only from the READY status projection. No naturalWidth, no getBBox, no DOM measurement and no Blob inspection anywhere on the path
APP3-S06 CANDIDATE_ORDER = STRUCTURE → COMPLEXITY → CONTEXT → GEOMETRY — the context step is the one APP3-S05 documented as unaskable without derivative authority; S06 is the checkpoint that has it
APP3-S06 PLACEMENT = ASPECT_PRESERVED + CENTRED_IN_AREA + NEVER_UPSCALED + WITHIN_PHYSICAL_MAXIMA — the operator-approved deterministic fallback, ruled on by APP3-P02 and refused rather than clamped when it does not fit
APP3-S06 REPLACE = NEW_ASSET + SAME_ELEMENT_ID + SAME_Z_ORDER + SAME_TRANSFORM — the old Asset is never deleted, and a replacement whose kept transform is no longer valid is refused with the prior element left exactly as it was
APP3-S06 RENDERER = ONE_NATIVE_SVG_IMAGE — an <image> in the same coordinate system as every other element. No HTML overlay, no canvas, no second SVG. The APP3-S02 placeholder remains, narrowed to media that is loading, refused or unavailable
APP3-S06 OBJECT_URLS = RUNTIME_ONLY — created per rendered derivative, revoked on replacement, Session change and unmount; never written into the document and never held in a store
APP3-S06 AUTOSAVE = NONE — no publicDesignSessionAutosave call, no timer, no saved indicator. The local Session revision is updated from the APP3-B06B response because the next mutation must present it; that is not a save
APP3-S06 PULLED_FORWARD = NONE — no layers, undo/redo, watermark, mobile touch editing, crop, flip, opacity, background removal, filter, download or export
APP3-S06 SURFACE = 37 paths / 42 operations / 84 schemas — measured from the generated artifact; the one new schema is the status projection
APP3-S06 MIGRATION = NONE
APP3-S06 DEPENDENCY = NONE
APP3-S06 ROOT_SCRIPTS = 30
...
FU-APP3-TEMPLATE-SOURCE-ASSET-INTAKE-01 = DEFERRED_LATER_APP3 — production Template image intake still has no owner; APP3-S06 did not implement it and APP3-E01 does not close it by fixturing bytes
APP3-S06-C1 = COMPLETE — REVIEW_ACCEPTED
APP3-S06-C1 CAUSE = INITIAL_IMAGE_PLACEMENT_IGNORES_PHYSICAL_MAX_DURING_CONSTRUCTION
APP3-S06-C1 FINDING = NOTHING_INSERTED_THEN_RESIZE_WITH_S03_IS_IMPOSSIBLE — the delivered rule fitted the Embroidery Area rectangle alone and left APP3-P02 to refuse the result whenever the Area's millimetre maximum was the tighter bound. The refusal is correct and unusable: no ImageElement exists, so there is nothing for APP3-S03 to resize
APP3-S06-C1 RULE = MIN(1, AREA_RECT/INTRINSIC, MM_TO_PX(AREA_MAXIMA)/INTRINSIC) — per axis, then the tighter scale, then centred in the Area. The millimetre maxima are resolved by the same areaAuthorityOf the final validation uses, so the "a null maximum means the Side is the only limit" rule has exactly one implementation
APP3-S06-C1 CONVERSION = APP3-P02 mmToPx — the sole px↔mm authority (IMP-D045 PO-10). No CSS pixel, no DPR, no 96 DPI, no local pxPerMm constant, no naturalWidth
APP3-S06-C1 CLASS = CONSTRUCTION_NOT_CLAMP — IMP-D045 PO-09 is unchanged: a transform the customer performed is still refused outright, never shrunk, snapped or re-centred. What changed is the system's own first candidate, which is derived from the authority before it exists rather than repaired after it fails
APP3-S06-C1 QUANTIZATION = P01_GRID_ROUNDED_DOWN — DESIGN_DOCUMENT_QUANTIZATION_SCALE, floored rather than nearest, because rounding a boundary-exact width to nearest can land one quantized unit outside the maximum it was built to respect
APP3-S06-C1 REFUSAL = NO_POSITIVE_BOX_ONLY — a degenerate rectangle, a non-positive maximum or an unusable px_per_mm. An Area merely tighter than its rectangle now places a smaller picture
APP3-S06-C1 CONTRACT = UNCHANGED — 37 paths / 42 operations / 84 schemas, 34 migrations, 30 root scripts, no API, worker, database, dependency or Figma change
APP3-S04 = COMPLETE — REVIEW_ACCEPTED — GROUP_DEFERRED_BY_AUTHORITY
APP3-S04 CLASS = FRONTEND_CAPABILITY_PARTIALLY_BLOCKED_BY_AUTHORITY — the layer list, z-order, lock and hide halves of the capability row are delivered; the group half is reported blocked rather than invented
APP3-S04 FIGMA = 3 ROWS APPROVED — FIG-STUDIO-LAYERS-DESKTOP-DEFAULT 608:3, -REORDER 608:68, -EMPTY 608:136, section 08 (596:14). No fourth frame, no Studio blanket approval, and FIG-STUDIO-EDITING-TABLET-1024 618:140 stays APP3-D01-C1's responsive reference
APP3-S04 FIGMA_MUTATION = NONE — no node created, moved or edited; only registry status changed
APP3-S04 FIGMA_READ = REGISTRY_AND_D01_ONLY — the Figma MCP connector requires interactive OAuth, which was not available in this run. Authority was resolved from FIGMA_DESIGN_INDEX.md, APP3-D01 §D and APP3-D01-C1 §H exactly as §4 permits; no node was guessed and no frame content was invented
APP3-S04 ZORDER = P01_ARRAY_ORDER — the element array is z-order, bottom first, and stays the only representation. The list is that array reversed once in the projection; the renderer is never reversed, sorted or re-ranked, and no zIndex field, layer array or CSS stacking number exists anywhere in the feature
APP3-S04 LIST = FLAT_TOP_FIRST — every element gets one row, groups included. The approved frames draw a flat list, so no expandable hierarchy affordance was invented; a grouped child is shown and its restack controls are disabled with a stated reason, because its position is not a top-level stacking decision
APP3-S04 REORDER = DRAG_PLUS_KEYBOARD — native DOM drag and drop with the approved drop indicator, and an adjacent move pair that reaches every z-order change the drag can reach. No interaction library, no dependency. A restack changes the element array and nothing else: no id, type, transform, flag, media field, text field, childIds or parent relation, and no API call
APP3-S04 LOCK = P01_LOCKED — the field is the whole model. The accepted editors already read it (APP3-S03 refuses a transform, APP3-S05 a text edit, APP3-S06 a replacement), so this checkpoint supplies the control and adds no second authority. Never pointer-events:none, and a locked group is not rewritten into its children
APP3-S04 HIDE = P01_VISIBLE — the element, its geometry, its z-order and its media are untouched and the row stays. Never a delete, a move to another list or an opacity of zero. A newly hidden selected element clears the selection, which is APP3-S02 §15's own rule applied at the moment the element stops qualifying
APP3-S04 GROUP = BLOCKED — the APP3-S04 capability row names group and this checkpoint could not implement it under accepted authority. Two independent grounds, either of which is sufficient; nothing was invented and no partial grouping shipped
APP3-S04 GROUP_FRAME = NOT_AUTHORIZED — IMP-D045 PO-07 says a grouping interaction "must choose the frame, rebase children into it and persist that explicitly", and defers the choice. The persisted box is the group's local frame AND its rotation/scale pivot (PO-03, PO-05), so a selection-AABB frame and a document-origin frame produce different behaviour the first time anyone rotates the group — and PO-12 makes group frame a schema-semantic change needing a Product Owner decision, a schema-version increase and a migration. There is no accepted document that determines it
APP3-S04 GROUP_MEMBER_SELECTION = NOT_AUTHORIZED — the three approved section-08 frames are List & Selection, Reordering and Empty. None draws a multi-select affordance, and APP3-S02's model is one selectedElementId. There is no approved interaction that can say which elements a group would contain, and inventing checkbox multi-select or stage multi-select is forbidden by APP3-S04 §15
APP3-S04 GROUP_EXISTING = REPRESENTED_NOT_CREATED — a group that already exists in a cloned Template document gets its own truthful row, its children are shown, and nothing restacks into or out of it. Reading a group is not creating one
APP3-S04 SELECTION = APP3-S02_UNCHANGED — one selectedElementId, single-select. The layer row is a second way to reach the same element, never a second selection model
APP3-S04 CANDIDATE = P01_STRUCTURE_THEN_COMMIT — every mutation builds an immutable candidate, APP3-P01 rules on it (which re-proves the group graph against the reordered array), and only a valid candidate is committed. APP3-P02 is deliberately not asked: PO-08 rules that z-order never affects bounds and that hidden and locked elements still have geometry, so none of the three mutations can change a geometric answer
APP3-S04 RENDER_REUSE = APP3-S03-C1_PRESERVED — a restack keeps every element instance, so shareDocumentIdentity reuses them and only the moved subtrees re-render; a flag change invalidates one element
APP3-S04 RESPONSIVE = 1440 EXACT / 1024 SHARED_DRAWER / 390 NO_EDITING — at 1024 the layer panel is a section of the one accepted 618:140 drawer behind the one APP3-S05-MI01 topbar trigger, exactly as APP3-D01-C1 anticipated ("layers merge into that same drawer"); never a second drawer. At 390 nothing that edits is rendered at all, because the mobile layer sheet is APP3-S11's
APP3-S04 AUTOSAVE = NONE — no publicDesignSessionAutosave call, no timer, no saved indicator. The API delta of every layer action after bootstrap is zero
APP3-S04 PULLED_FORWARD = NONE — no undo, redo or history; no watermark; no autosave, conflict or resume UI; no mobile or touch layer surface; no duplicate, delete, align or distribute
APP3-S04 SURFACE = 37 paths / 42 operations / 84 schemas — unchanged; no backend work
APP3-S04 MIGRATION = NONE
APP3-S04 DEPENDENCY = NONE
APP3-S04 ROOT_SCRIPTS = 30
FU-APP3-S04-GROUP-AUTHORITY-01 = OPEN — NONBLOCKING — group and ungroup need a Product Owner ruling on the persisted group frame and pivot (IMP-D045 PO-07/PO-12) and an approved member-selection interaction, before any checkpoint can implement them
APP3-S09 = COMPLETE — REVIEW_ACCEPTED
APP3-S09 CLASS = FRONTEND_RUNTIME_ONLY_CAPABILITY — a presentation layer over the accepted preview; no document field, no API, no worker, no database
APP3-S09 FIGMA = 4 ROWS APPROVED — FIG-STUDIO-WATERMARK-DESKTOP-LIGHT 609:263, -DARK 609:299, -MOBILE-DEFAULT 609:335, FIG-STUDIO-WATERMARK-POLICY 609:371, section 13 (596:19). No fifth frame, no Studio blanket approval; S08, S10 and S11 rows stay REVIEW_REQUIRED
APP3-S09 FIGMA_MUTATION = NONE — no node created, moved or edited; only registry status changed
APP3-S09 FIGMA_READ = REGISTRY_AND_D01_ONLY — the Figma MCP connector needs interactive OAuth, unavailable in this run. Authority came from FIGMA_DESIGN_INDEX.md and APP3-D01 §I.4, which records the approved treatments exactly: ink at ~13% over light imagery, white at ~22% over dark, never part of the document, no toggle, no download or export control anywhere, and a policy note that explicitly does not claim screenshots can be prevented
APP3-S09 DOCUMENT = NOT_SERIALIZED — APP3-P01 gains no watermark element, no root field, no token and no coordinates. The watermark is unrepresentable in a DesignDocument rather than merely absent from one, so it cannot enter the canonical hash, the APP3-S04 layer list, APP3-S02 selection, APP3-S03 transforms or a future APP3-S10 save. Proved by canonicalizing the document before and after the overlay mounts and comparing the strings
APP3-S09 LAYER = VIEWPORT_SIBLING_OF_THE_S07_TRANSFORM — the overlay is a sibling of studio-stage__viewport-layer inside studio-stage__viewport, so it covers the visible preview at every zoom step and pan offset and is clipped by the viewport own overflow. Anchoring it to document coordinates would leave the visible preview bare the moment the visible preview stops being the whole document
APP3-S09 TOKEN = CRYPTO_RANDOM_ONCE_PER_RUNTIME — 8 characters from an unambiguous 32-character alphabet, minted by crypto.getRandomValues in a useState initialiser. Math.random is never a fallback: an environment without a cryptographic source gets a constant marked as unavailable, because a value meant to be unguessable and quietly not is worse than an obvious refusal
APP3-S09 TOKEN_PII = NONE — mintWatermarkToken takes no argument, so there is no path by which a name, email, phone, IP, user agent, Session id or secret, storage key, assetId or derivativeId could reach it. Never sent, never persisted, never logged, and it authorizes nothing
APP3-S09 CONTRAST = BOTH_APPROVED_TREATMENTS_DRAWN — each mark is drawn twice, white at 22% beneath and ink at 13% one pixel above, so legibility on light and on dark imagery is met by construction. The alternative — sampling the customer photograph to choose a treatment — would read customer pixels every frame on the one surface ADR-APP0-001 measured, and this checkpoint has no authority to do it
APP3-S09 PATTERN = FIXED_35_MARKS — 7 rows x 5 columns, rotated -30 degrees, over-drawn from -25% to 125% so the rotated corners are covered. The count is a function of the viewport grid alone: not the element count, not the zoom factor, not the pan offset, not the number of image layers
APP3-S09 SELECTABILITY = NONE — absent from selection, transform, the layer list, reorder, lock, hide, text editing and image replacement, because it is absent from the document all of those operate on. pointer-events:none, user-select:none and aria-hidden are presentation on top of that, not the guarantee
APP3-S09 POLICY = TWO_FACTS_NO_PROMISE — the preview is marked and there is no download. Nothing about screenshots, screen recording, copying or printing, and no PrintScreen trap, blur trick, contextmenu block or canvas-taint hack anywhere: docs/09-SECURITY-AND-ABUSE-PREVENTION.md §6 ends with do not claim absolute screenshot prevention
APP3-S09 EXPORT = NONE — no download, export, print, share or copy-image control was added or exposed
APP3-S09 RENDERER = ONE_SVG_UNCHANGED — the watermark is DOM and CSS. No second SVG root, no canvas, no new dependency, and APP3-S02 own one-renderer rule still passes unmodified
APP3-S09 API = ZERO — no operation added, called or curated. No autosave, no APP3-B06C refetch caused by the watermark, no analytics and no tracking
APP3-S09 SURFACE = 37 paths / 42 operations / 84 schemas — unchanged
APP3-S09 MIGRATION = NONE
APP3-S09 DEPENDENCY = NONE
APP3-S09 ROOT_SCRIPTS = 30
APP3-S08 = COMPLETE — REVIEW_ACCEPTED — CORRECTED_BY_APP3-S08-C1
APP3-S08 CLASS = FRONTEND_RUNTIME_ONLY_CAPABILITY — local undo and redo over the accepted working document; no API operation, no worker, no database, no dependency
APP3-S08 FIGMA = 2 ROWS APPROVED — FIG-STUDIO-UNDO-DESKTOP-MIDHISTORY 609:147, FIG-STUDIO-UNDO-DESKTOP-DISABLED 609:209, section 12 (596:18). No third frame, no Studio blanket approval; S10 and S11 rows stay REVIEW_REQUIRED
APP3-S08 FIGMA_MUTATION = NONE — no node created, moved or edited; only registry status changed
APP3-S08 FIGMA_READ = REGISTRY_AND_D01_ONLY — the Figma MCP connector needs interactive OAuth, unavailable in this run. Authority came from FIGMA_DESIGN_INDEX.md and APP3-D01 §D, which records the section exactly: undo-redo with a history list, a disabled state and keyboard hints, and a directive not to imply an infinite history
APP3-S08 MODEL = DOMAIN_DOCUMENT_SNAPSHOTS — ADR-APP0-001 §6 and IMP-D026 both rule that undo/redo is a domain command/snapshot concern and never an engine history stack. An entry is two APP3-P01 documents, one closed action kind and one bounded label. No DOM node, SVGElement, APP3-P02 graph, Blob, object URL, query result, upload progress, Session credential, selection, zoom, pan or watermark token can enter one, because a DesignDocument is closed over JSON scalars by construction
APP3-S08 CURRENT_DOCUMENT = ONE — the past and the future live in the same store as the one working document rather than in a controller with a current of its own. undo and redo write document and move the cursor in one set, so the two cannot disagree even for one render. No historyCurrentDocument and no second editable scene
APP3-S08 BOUND = 50 — MAX_HISTORY_ENTRIES. No accepted numeric bound exists in any authority, so this is the APP3-S08 §6 operator fallback: an in-memory engineering limit for one browser tab, in no API, no document field and no database column. The oldest entry is discarded when it is reached, and the panel states the number because APP3-D01 forbids implying an infinite history
APP3-S08 COALESCING = GESTURE_AND_TEXT_SESSION — one drag, resize or rotate is one entry however many pointer frames it took; one text editing session between focus and blur is one entry however many keystrokes or IME frames it took. A refused gesture, a gesture that ended where it started and a composition that was never committed each add zero. No debounce timer is a boundary anywhere
APP3-S08 PER_FRAME_COST = ZERO — the baseline is captured once at pointerdown, which is the snapshot APP3-S03 already froze. No JSON.parse(JSON.stringify()), no structuredClone and no allocation on any pointer frame; the document comparison runs once, at the boundary, and short-circuits on reference equality
APP3-S08 ATOMIC = REORDER_LOCK_HIDE_TEXT_FORMAT_IMAGE — each is one entry. A candidate an authority refused never reaches the store, so a refusal appends nothing
APP3-S08 REDO_CLEARED_BY = A_NEW_FORWARD_MUTATION_ONLY — a refused or no-op attempt leaves the future intact
APP3-S08 SELECTION = NOT_HISTORY — never snapshotted and never restored. A selection an undo left pointing at nothing is cleared through the existing APP3-S02 reconciliation, and a redo fabricates none
APP3-S08 VIEWPORT_AND_WATERMARK = UNCHANGED_BY_HISTORY — no APP3-S07 zoom or pan and no APP3-S09 token in a snapshot or a label; undo and redo change neither
APP3-S08 SIDE_EFFECTS = NONE — undo and redo call no operation at all. No APP3-B08 autosave, no Session revision movement, no Asset delete, no re-upload, no worker or storage effect. Undoing a placement removes a local element; the durable Asset the upload created is untouched, and redo restores the same assetId and derivativeId rather than uploading again
APP3-S08 PERSISTENCE = NONE — no localStorage, sessionStorage, IndexedDB, cookie or URL. History is local to one Studio runtime and one Session: a different Session clears it and a reload starts from the persisted Session document with nothing to undo. APP3-S10 owns persistence and resume
APP3-S08 SHORTCUTS = CTRL_CMD_Z_AND_CTRL_CMD_SHIFT_Z — exactly the two the approved hints name. Ctrl+Y is deliberately unbound: no authority and no existing convention establishes it. keydown, defaultPrevented respected, listener removed on unmount, and the handler stands down entirely inside an input, textarea, select or contenteditable so the platform's own text undo keeps working there
APP3-S08 HISTORY_LIST = INFORMATIONAL — rows say what happened and which one the design is currently at. No click-to-time-travel: APP3-S08 §14 rules that the presence of a list is not authority for arbitrary jumping, and the two approved frames name states that stepping with the two controls already produces
APP3-S08 MOBILE = NONE_PRE_S11 — 390 renders no undo surface, only the honest sentence the other capabilities render. At 1024 the history detail is one more section of the accepted 618:140 drawer, so there is still one topbar, one trigger and one drawer; the controls stay in the persistent left tool rail, which 618:140 keeps visible in its own callout
APP3-S08 GROUP = ABSENT — no group action kind exists, because no group capability does. FU-APP3-S04-GROUP-AUTHORITY-01 stays open
APP3-S08 SURFACE = 37 paths / 42 operations / 84 schemas — unchanged; no backend work
APP3-S08 MIGRATION = NONE
APP3-S08 DEPENDENCY = NONE
APP3-S08 ROOT_SCRIPTS = 30
APP3-S08 BENCHMARK = L100_CHROMIUM_IDENTICAL_TO_HEAD — move p50 16.7 p95 33.4, resize p50 16.7 p95 16.7, rotate p50 16.7 p95 33.4, measured back to back against APP3-S09-accepted HEAD on the same machine, where HEAD is 33.4 / 33.3 / 33.4. APP3-S08 adds no measurable per-frame cost. The 20 ms p95 budget is breached at HEAD as well, which APP3-S03-C1 recorded at 16.7 — raised as FU-APP3-TRANSFORM-BUDGET-01, non-blocking, and not attributed to any checkpoint without a bisect. WebKit not run: a second pair does not fit the IMP-D043 PO-07 five-Session hourly cap, and the counter was not cleared to make it fit
FU-APP3-TRANSFORM-BUDGET-01 = OPEN — NONBLOCKING — the L=100 Chromium transform p95 is 33.4 ms at APP3-S09-accepted HEAD against the frozen ADR-APP0-001 20 ms budget, where APP3-S03-C1 recorded 16.7 ms. Every Studio capability checkpoint since carries benchmark = no. Attribution needs a bisect of APP3-S03-C1 through APP3-S09 under the PO-07 Session cap, plus a WebKit measurement
FU-APP3-S08-DESIGN-READ-01 = COMPLETE — CLOSED_BY_HUMAN_REVIEW_LIVE_FIGMA_READ — human review opened 609:147, 609:209 and 618:140 with the live connector and returned their exact structure and geometry to APP3-S08-C1. The engineering placements the follow-up recorded were confirmed wrong on two points and corrected there; nothing about the nodes remains unread
APP3-S08-C1 = COMPLETE — REVIEW_ACCEPTED
APP3-S08-C1 CAUSE = TWO_FIGMA_FIDELITY_DEFECTS_FOUND_AT_HUMAN_REVIEW — the architecture, behaviour, coalescing, bound, keyboard semantics and side-effect absence of APP3-S08 were accepted and are not reimplemented. What was wrong was fidelity, and only because the delivering run could not open the nodes
APP3-S08-C1 FIGMA_READ = LIVE_NODES_609_147_AND_609_209_AND_618_140 — supplied by human review from the live connector: section 12 (596:18) of page APP_03 in file BQwqV8GdfUIELvsQDB1UQE. No node was created, moved or edited by this checkpoint
APP3-S08-C1 DEFECT_1 = CONTROLS_WERE_IN_THE_PANEL_HEADER — both frames draw a persistent left tool rail (x=0 y=60 w=72 h=840) carrying Undo at y=132 and Redo at y=188, with the history panel a separate right-side surface at x=990 y=120 w=400 h=300 and the shortcut hint its own box at y=444. The delivered panel header held the controls and the hint. Corrected: the controls are in the rail, the panel carries no control at all, and the hint is its own informational section
APP3-S08-C1 DEFECT_2 = PER_ROW_APPLIED_UNDONE_STATUS — the delivered list showed a fresh history as zero rows and labelled every row Đang áp dụng or Đã hoàn tác. 609:209 draws a non-empty Nothing-to-Undo panel with one non-undoable baseline row carrying the badge hiện tại, and 609:147 marks only the current position. Corrected: a baseline display row, exactly one hiện tại, and no per-row status anywhere
APP3-S08-C1 RAIL_SCOPE = UNDO_AND_REDO_ONLY — the rail the frames draw also carries the text and image tools. Those are APP3-S05 and APP3-S06 delivered and accepted surfaces, and a correction that owns neither does not re-author two accepted checkpoints. Raised as FU-APP3-STUDIO-TOOL-RAIL-01
APP3-S08-C1 ENGINE = UNCHANGED — the baseline is a presentation projection and never an entry. entries and cursor, MAX_HISTORY_ENTRIES = 50, recordAction, undo, redo, openAction, the gesture and text coalescing and the store are byte-identical in behaviour; a fresh runtime is still entries.length 0 and cursor 0, and undo is still disabled there
APP3-S08-C1 CURRENT_MARKER = EXACTLY_ONE_BY_CONSTRUCTION — cursor 0 marks the baseline, cursor n marks the action row at index n-1. Both are computed from the one cursor, so two markers and none are unrepresentable rather than merely untested
APP3-S08-C1 FUTURE_ROWS = VISIBLE — the rows past the marker stay in the list after an undo. They are what redo returns to, and hiding them would leave the redo control pointing at nothing a customer can see
APP3-S08-C1 BASELINE_COPY = LINEAGE_GATED_DISPLAY_NAME_ONLY — a clone whose Template display name APP3-S01 already fetched shows Mở từ mẫu “<bounded name>”, bounded by the same LABEL_MAX_LENGTH the layer panel uses and matched against the Session's own lineage slug so a name from a different Template cannot be presented as this one's provenance. A blank Session reuses the accepted APP3-S01 startBlank label. No API call, no P01 field, nothing inside a snapshot
APP3-S08-C1 BASELINE_GENERIC_COPY = PRESENTATION_FALLBACK_FOR_RUNTIME_WITHOUT_DISPLAY_NAME — Mở từ mẫu có sẵn, for a resumed clone whose lineage carries a slug and a version and no name. It exposes no technical identity: no slug, no version, no id and no digit
APP3-S08-C1 MOBILE = ZERO_S08_CONTROLS_AT_390 — no rail, no undo, no redo, no history panel, no shortcut block and no APP3-S11 bottom sheet. The approved desktop note delegates the mobile controls to APP3-S11 in its own words, and that sentence is a desktop string
APP3-S08-C1 SHELL = ONE_GRID_COLUMN_ADDED — studio-stage__frame became a two-column grid so a persistent rail can sit beside the stage. Every existing child keeps its order and its full width in the second column; the auto rail track collapses to zero where React renders no rail, so 390 is unchanged. No S02–S09 control was moved, renamed or restyled
APP3-S08-C1 PERFORMANCE = PERFORMANCE_RESULT_REUSED_FROM_APP3_S08 — no transform-frame semantics changed: the gesture path, the store commit, the scene adapter and the memoized element are untouched, and the rail renders two buttons whose props cannot change during a coalesced action. No new benchmark Session was opened and no rate-limit window was waited on or cleared
APP3-S08-C1 SURFACE = 37 paths / 42 operations / 84 schemas — unchanged; no backend work
APP3-S08-C1 MIGRATION = NONE
APP3-S08-C1 DEPENDENCY = NONE
APP3-S08-C1 ROOT_SCRIPTS = 30
APP3-S08-C1 BROWSER_PROOF = ONE_CLONE_SESSION_1440_1024_390 — one real Session on the real route. 1440: rail x=150 w=48 with Undo y=27 and Redo y=83 both 48x48, panel at x=210 to its right, shortcut hint at y=1437 below the panel and outside it, zero controls in the panel, both really disabled with no aria-disabled, baseline row reading Mở từ mẫu “Đèn lồng Hội An”. Three actions then three undos and one redo: the marker moved 3 → 2 → 1 → 0 → 1, the row count stayed 4 at every step so no future row was hidden, exactly one hiện tại throughout, Undo disabled only at the baseline and Redo enabled as soon as a future existed. 1024: rail persists at 48x48 outside the drawer, one drawer holding the list and the hint, stage box byte-identical open and closed, no horizontal overflow. 390: zero rail, undo, redo, list, hint, baseline and bottom sheet, only the accepted pre-S11 notice, no horizontal overflow. Zero /api requests from any history operation and zero Studio application-storage keys
APP3-S08-C1 FIXTURE_NOTE = TEMPLATES_WERE_ARCHIVED_BY_A_PREVIOUS_REVERT — the APP3-S01 smoke fixture seeds Templates with on conflict (id) do nothing, so a seed after a revert leaves them ARCHIVED and the picker has nothing to clone. The rows were set back to PUBLISHED for the run through the fixture module's own helper and returned to ARCHIVED afterwards; the development database is as it was found. Raised as FU-APP3-S01-FIXTURE-IDEMPOTENCY-01 rather than repaired here
FU-APP3-S01-FIXTURE-IDEMPOTENCY-01 = COMPLETE — CLOSED_BY_APP3-E01 — the APP3-S01 fixture seeded design_templates with on conflict (id) do nothing while revert archives them, so a second seed left every Template ARCHIVED and the picker had nothing to clone. Repaired in the FIXTURE lifecycle only: the insert now restores the state it intends (status PUBLISHED, archived_at null, current_version 1), exactly as the Side and Area inserts beside it already did. No production Template lifecycle rule changed. Proved by running revert then seed twice with no manual SQL between the cycles: published templates on the fixture placement = 14 both times, where the second cycle previously reported 0 to the public reads
FU-APP3-STUDIO-TOOL-RAIL-01 = OPEN — NONBLOCKING — 609:147 and 618:140 draw the text and image tools in the same persistent left rail APP3-S08-C1 introduced for undo and redo. They are currently APP3-S05 and APP3-S06 panel surfaces, already delivered and accepted, and moving them is a composition change for a checkpoint that owns them rather than for a correction that owns neither
APP3-S08-C1 REVIEW_OUTCOME = ACCEPTED_FOR_PROGRESSION — human review ruled NO MORE S08 CORRECTION: no APP3-S08-C2, no APP3-S08-MI01 and no other S08 extension checkpoint exists or may be created. Any S08 presentation or shell issue that becomes mechanically necessary to finish the next capability is owned inside APP3-S10
S08_CARRY_FORWARD_RECONCILIATION_OWNER = APP3-S10 — a bounded ownership transfer, not a reopening of APP3-S08 and not a correction lineage
APP3-S10 = COMPLETE — REVIEW_ACCEPTED
APP3-S10 ACCEPTED_AT = ccf15f7 feat(storefront): add Studio autosave recovery UX
APP3-S10 CLASS = FRONTEND_CAPABILITY_OVER_ACCEPTED_APP3-B08 — autosave, save-state, conflict, resume and expiry UX consuming one already-published operation; no new API operation, no worker, no database and no dependency
APP3-S10 FIGMA = 6 ROWS APPROVED — FIG-STUDIO-AUTOSAVE-DESKTOP-SAVED 610:3, -SAVING 610:41, -OFFLINE 610:77, -CONFLICT 610:118, -RESUME 610:159, -EXPIRED 610:201, section 14 (596:20). No seventh frame, no Studio blanket approval, and the six APP3-S11 mobile rows stay REVIEW_REQUIRED
APP3-S10 CADENCE = 2500_MS_QUIET_UNDER_A_10000_MS_CEILING — the value APP3-D01 left open and APP3-G04 §6.7 routed here. A save becomes eligible at min(last mutation + 2500, first dirty mutation + 10000), so a continuously edited document cannot postpone its own save; the worst case is one save every 10 s, which is 6/minute against the IMP-D043 ceiling of 30
APP3-S10 IN_FLIGHT = ONE — a save owns everything dirty up to the instant it starts, an edit arriving during it begins a new streak, and the next save is scheduled only after the current request settles. No setInterval, no polling, no background sync, no service worker and no save from beforeunload
APP3-S10 REVISION = SERVER_ONLY — expectedRevision is always a revision the client read from a response. Nothing computes previous + 1
APP3-S10 SUCCESS = CANONICAL_ADOPTION_WITHOUT_HISTORY — a response with no newer local edit adopts the server's canonical document through a non-history seam; a response that lost a race leaves the newer local document alone and schedules the next save. Neither is an undoable entry, because the customer did neither
APP3-S10 CONFLICT = TWO_CHOICES_NO_MERGE — 409 stops autosave, reads the latest Session state exactly once, and shows the two actions 610:118 authorizes. Tải bản mới nhất adopts the server document and resets the local past with the branch it described; Giữ bản trên màn hình keeps both and performs one user-initiated save against the revision the server actually holds. No third button, no automatic merge, no background overwrite, and a second 409 re-enters the same decision rather than looping
APP3-S10 AMBIGUOUS = RECONCILE_BEFORE_REPLAY — a save with no HTTP response at all may have reached the server, so the latest snapshot is read first and the outcome decided from it: the submitted document present at a higher revision is treated as saved, the base revision unchanged is retryable, and anything else is a conflict. No blind replay exists in any path
APP3-S10 RETRY = BOUNDED_5000_15000_THEN_ERROR_PAUSED — two automatic attempts and then nothing until the customer presses Thử lưu lại or makes a new edit. Tiếp tục thiết kế dismisses the message and never the truth: the state, the failure and the navigation warning are untouched and nothing is marked saved
APP3-S10 ERRORS = STATUS_ONLY_TAXONOMY — 401 expired, 403 refused, 409 conflict, 413/422 rejected, 429 throttled, 5xx server, no response ambiguous. No message text is branched on and no backend internal reaches a customer
APP3-S10 RESUME_HANDLE = NON_SECRET_SESSION_ID_ONLY — one localStorage key namespaced by product slug, Side code and Area code, holding the Session id and nothing else. Never the secret (HttpOnly, unreadable), a hash of it, the document, the revision, expiresAt as authority or any customer identity; the value is re-validated against the id shape on the way out because browser storage is writable by anything on the origin
APP3-S10 RESUMED_SCOPE = PUBLIC_PLACEMENT_MANIFEST_FOR_THE_HANDLE_NAMESPACE — APP3-B07 omits scope on resume, so a Session reopened after a full reload has no geometry to draw from. Every field of DesignSessionScopeResponse is published verbatim by the placement manifest APP3-S01 already fetched, and each is copied — nothing computed, scaled or defaulted — for the exact placement the handle was written under
APP3-S10 EXPIRY = SERVER_AUTHORITY_NO_RECOVERY — a 401 clears the matching handle, stops autosave and shows 610:201 with its two approved ways forward. No TTL is extended from the browser, no cookie is scanned, no grace secret exists and nothing claims the old Session is recoverable
APP3-S10 HISTORY = RESET_ONLY_ON_BRANCH_REPLACEMENT — a resume the customer accepted and a load-latest conflict resolution reset the APP3-S08 past, because it described a branch that no longer leads to what is on screen. A canonicalized save response does not. Undo and redo are ordinary document mutations and are autosaved on the same cadence; no history is persisted and no entry is coupled to a revision
APP3-S10 SHELL = ONE_TOPBAR_ONE_RAIL_ONE_DRAWER_ONE_STAGE — the .studio-stage__topbar wrapper moved from inside the tablet text drawer to the stage frame, because the save chip belongs in that region at every tier and a topbar that existed only at 1024 could not hold it. The trigger's markup, state, label and behaviour are unchanged, and no S02–S09 control was moved, renamed or restyled
APP3-S10 STATE_SURFACE_PLACEMENT = UNDER_THE_TOPBAR_AT_EVERY_TIER — a deviation from the 1440 frames, which draw the state as a right-hand panel. Two of its four states are decisions about losing unsaved work, the one accepted right-hand region at 1024 is a drawer that is closed by default, and 390 has no right-hand region at all. Disclosed rather than taken quietly
APP3-S10 CONFLICT_EYEBROW = OMITTED_RAW_STATUS_CODE — 610:118 draws a "409 · xung đột bản sửa" eyebrow above its heading. A status code is a backend internal, so the heading carries the meaning and the code is not shown to a customer
APP3-S10 NON_NETWORK_FAILURE_COPY = ADDED_TRUTHFUL_VARIANT_OF_610_77 — the approved offline sentence names the connection, which is only true when the connection failed. A refused, throttled or broken save keeps the same shape with the cause left unnamed rather than named wrongly
APP3-S10 MOBILE = TRUTHFUL_STATE_ONLY_AT_390 — the save chip renders, because a customer whose work is unsaved must be told. No bottom sheet, no mobile toolbar, no touch gesture and no editing surface: APP3-S11 still owns every one of them
APP3-S10 SURFACE = 37 paths / 42 operations / 84 schemas — unchanged; no backend work. publicDesignSessionAutosave crossed the curated client boundary on exactly the terms its withholding stated
APP3-S10 MIGRATION = NONE
APP3-S10 DEPENDENCY = NONE
APP3-S10 ROOT_SCRIPTS = 30
APP3-S10 BROWSER_PROOF = SIX_JOURNEYS_ON_A_REAL_SESSION — Playwright against /san-pham/a03-live-check-redirect/thiet-ke on the development stack over the APP3-S01 trustworthy origin, two anonymous Sessions total (inside the IMP-D043 PO-07 ceiling of 5/hour/IP; no API restart and no real rate-window sleep). (A) one typing burst → Chưa lưu at +0.27 s → one PUT 200 at +2.64 s → "Đã lưu lúc 12:40" from a real response: one write for the whole burst. (B) an edit made while Đang lưu… was on screen survived the in-flight response, produced a second PUT, and the server row ended at autosave_revision 3 holding the newer text. (C) a genuine two-tab 409: PUT 409 → exactly one POST /resume read → Xung đột with exactly Tải bản mới nhất and Giữ bản trên màn hình and no write; load-latest replaced the stage with the server's document and left Hoàn tác disabled (the past that described the abandoned branch is gone); a second 409 resolved with Giữ bản trên màn hình kept document and history and wrote once, landing autosave_revision 6. (D) intercepted transport failures (net::ERR_CONNECTION_RESET): Mất kết nối, each failed attempt followed by exactly one reconciliation read, ladder gaps measured at +5.03 s and +15.04 s and then silence for the remaining 9 s, ending on the ERROR_PAUSED surface with Thử lưu lại and Tiếp tục thiết kế; pressing Thử lưu lại with the network restored saved immediately. (E) a full reload offered Khôi phục phiên from one localStorage key `embroidery.studio.session:a03-live-check-redirect:kkkk:chest` whose entire value is the Session id — Tiếp tục resumed and drew the placement, Bắt đầu lại cleared the key. (F) a Session invalidated server-side produced one PUT 401 → 610:201 with its eyebrow and two actions, no retry, no reconciliation read, and the handle cleared. Shell composition recorded at 1024: exactly one .studio-stage__topbar (holding the chip), one .studio-history-rail with 48×48 controls, one .studio-drawer and one .studio-stage__frame — the drawer is position:absolute and the stage rect is byte-identical before and after it opens. At 390: one topbar, the chip, one save-state region, zero APP3-S11 mobile surfaces and no horizontal overflow
APP3-S10 RETRY_EXTRA_ATTEMPT = DISCLOSED_OBSERVATION — when the failing streak begins with a customer edit rather than a pressed Thử lưu lại, the browser run measured one additional attempt one debounce (≈2.5 s) after the first failure, before the [5000, 15000] ladder ran from the second. Measured, not inferred: attempts at +2.53 s, +5.10 s, +10.13 s, +25.17 s, then stop. Both properties §16 requires still hold — the ladder is bounded and it stops — and the extra attempt is one request, reconciled like every other. The mechanism was not isolated inside this checkpoint and the unit suites (fake clock, no live streak) do not reproduce it, so it is carried as FU-APP3-S10-RETRY-EXTRA-ATTEMPT-01 rather than claimed away
APP3-S10 PERFORMANCE = NO_PER_FRAME_WORK_ADDED — the dirty streak, the in-flight save, the retry attempt and the server revision are refs, not state, so a gesture committing the working document every pointer frame re-renders nothing for autosave; only a state *transition* renders. Evidenced end-to-end by (A): a full typing burst produced exactly one request. No new benchmark was run and FU-APP3-TRANSFORM-BUDGET-01 stays open and un-bisected
FU-APP3-S10-RETRY-EXTRA-ATTEMPT-01 = COMPLETE — CLOSED_BY_APP3-E01 — VERDICT_EXPECTED_BY_STATE_MACHINE. Reproduced on the accepted APP3-S10 fake clock, and the timelines answer it mechanically: a failing streak from one edit attempts at [2500, 8000, 23000] ms — a debounce, then the locked [5000, 15000] ladder, then stop; from a pressed Thử lưu lại it attempts at [0, 6000, 21000] — the ladder alone. Neither has a fourth attempt. A streak carrying a SECOND document mutation attempts at [2500, 5500, 11000, 26000] — four, structurally identical to the four APP3-S10 measured in the browser (+2.53, +5.10, +10.13, +25.17). The extra attempt is one more EDIT being saved, not an extra retry: S10 re-arms the ladder on a new mutation by design, so a spent one from a network blip cannot leave the rest of a session unsaved. The browser run typed into a text field and APP3-S05 coalesces a typing burst into an edit session that closes on its own, which is the second mutation. Every shape is bounded, stops in ERROR_PAUSED, and performs exactly one reconciliation read per attempt. No production change was needed and none was made
FU-APP3-STUDIO-TOOL-RAIL-01 = STILL_OPEN_AFTER_APP3-S11 — S10 relocated the .studio-stage__topbar wrapper and S11 gave 390 one coherent tool surface of its own, but neither moved the APP3-S05 text and APP3-S06 image tools into the **desktop and tablet** persistent rail, which is what 609:147 and 618:140 draw and what this follow-up is about. S11 was named a candidate owner and is not closing it by prose: at 1440 and 1024 the tools are still panel surfaces, so the follow-up stays open for the checkpoint that moves them
APP3-S11 = COMPLETE — REVIEW_ACCEPTED
APP3-S11 ACCEPTED_AT = 9433693 feat(storefront): add Studio mobile touch editing
APP3-S11 CLASS = FRONTEND_COMPOSITION_AND_INPUT_OVER_ACCEPTED_S02_S10 — mobile access at 390 to capabilities that already exist; no new API operation, no worker, no database, no dependency and no new P01/P02 semantics
APP3-S11 FIGMA = 6 ROWS APPROVED — FIG-STUDIO-MOBILE-STAGE-SELECTED 610:242, -TRANSFORMSHEET 610:294, -LAYERSSHEET 610:353, -TEXTSHEET 610:409, -IMAGESHEET 610:465, -CONFLICT 610:514, section 15 (596:21). No seventh frame and no Studio blanket approval. This release closes the last Studio section, so every Studio row now belongs to a checkpoint that consumed it
APP3-S11 DESIGN_AUTHORITY = OPERATOR_LIVE_READ_TRANSCRIPTION — the six frames were opened live during human review and their contents transcribed into the APP3-S11 directive §§5–14. The Figma MCP connector needs an interactive OAuth this run did not have, so the frames were not re-opened here; the registry rows, their node ids and the transcribed annotations are the authority used, and no Figma file was mutated
APP3-S11 GESTURES = ONE_FINGER_IS_THE_DESIGN_TWO_ARE_THE_CAMERA — arbitration is a total function of the live touch count, decided the instant a finger lands rather than inferred from movement. One touch on a selected editable element drives the accepted APP3-S03 candidate path; two or more drive the APP3-S07 viewport and nothing else. A third finger starts no second gesture
APP3-S11 PINCH = DISCRETE_STEP_NEVER_A_SCALE — a pinch resolves to an index into the frozen APP3-S07 zoom list [1, 1.25, 1.5, 2, 3, 4], nearest in log space, from the separation and step the gesture began with. ADR-APP0-001 measured its 41 ms WebKit p95 by driving a continuous pinch, so the gesture that produced that number does not get an exception: an arbitrary scale stays unrepresentable. A 24 px separation floor and an 8 % dead zone keep a two-finger pan from flickering between steps
APP3-S11 VIEWPORT_GESTURES = ZERO_DOCUMENT_ZERO_HISTORY_ZERO_AUTOSAVE — pinch and two-finger pan reach the APP3-S07 store and stop. Moving the camera is not editing the design
APP3-S11 ARBITRATION_HANDOFF = S03_CANCELLATION_SEMANTICS — a second finger landing during an element drag closes that drag through the same path an unmount takes, so whatever the last valid frame committed becomes exactly one history entry, or none. Nothing is half applied, because every frame of a drag committed a whole valid document or nothing
APP3-S11 TOUCH_SEAM = ONE_CAPABILITY_FLAG_NOT_A_SECOND_PATH — useStudioTransform gained a `touch` option, off at every tier but mobile. A touch drag produces the same candidate, through the same APP3-P02 validation, closing the same single APP3-S08 action; the flag decides only whether a pointer may start one
APP3-S11 HANDLES = FOUR_CORNERS_THAT_OPEN_THE_SHEET — 610:242 draws ~22 px knobs at the four corners and no rotate affordance, and no approved annotation authorizes direct corner-handle resize by touch. So at 390 a knob is a ≥44 px button that opens 610:294 rather than a drag target, the eight desktop handles are not reproduced, and no touch rotation gesture was invented
APP3-S11 TRANSFORM_SHEET = BUTTONS_INTO_THE_ACCEPTED_AUTHORITY — every ± press builds a candidate and puts it through APP3-P01 structure, quantization, APP3-P02 containment and APP3-P02 physical size, exactly as a drag does. No clamping, no snapping, no warn-but-persist. One accepted press is one history entry; a refused one commits nothing and appends nothing
APP3-S11 TRANSFORM_READOUT = MEASURED_NOT_ECHOED — the mm figures are the stroke-aware transformed AABB through the Side's pxPerMm, re-measured after every accepted press. IMP-D045 PO-04 makes scale the persisted resize model and states it never alters width/height, so a millimetre button cannot write a millimetre: it writes the scale that moves the measured size by about that much, and the read-out says what it became
APP3-S11 TRANSFORM_STEPS = PRESENTATION_ONLY_1MM_AND_1DEG — no accepted authority states a UI increment, so this checkpoint chose one and says so. Both are far above the APP3-P01 four-decimal quantization floor, so no press is rounded away to nothing. Neither is a persisted precision
APP3-S11 LAYERS_SHEET = S04_CONTROLLER_WITH_A_TOUCH_PATH — native HTML drag events do not fire for touch, so the sheet drives the same startDrag/dragOver/drop the desktop list does, from pointer events, resolving the row under the finger with the browser's own hit test. Only the explicit ⋮⋮ handle starts a reorder, so scrolling a list cannot restack a design by accident; the two step buttons stay beside it because APP3-S04 §9 forbids a drag being the only way. One drop is one entry. No group or ungroup
APP3-S11 TEXT_SHEET = S05_INSPECTOR_UNCHANGED — the same component the desktop panel renders, in a sheet. EDIT_ONLY_NO_CREATION holds: the toolbar's T opens the sheet for a selected text element and is disabled otherwise, and it never became an Add Text button. The exact controlled-font triple, the IME rules and the P01 text limits are S05's and are untouched
APP3-S11 KEYBOARD = MEASURED_VISUAL_VIEWPORT_INSET — 610:409 promises the sheet is pushed up rather than covered, and a note is only true if the build makes it true. The inset is measured from VisualViewport while the text sheet is open, never a constant: keyboard heights differ by device, language and keyboard. A browser without the API reports 0 and nothing moves on a guess
APP3-S11 IMAGE_SHEET = S06_INSPECTOR_UNCHANGED — one upload path, one state machine, one API. MOBILE_IMAGE_COPY_RECONCILIATION = ALIGNED_TO_ACCEPTED_B06B_CONTRACT: 610:465 illustrates "PNG hoặc JPG, tối đa 10 MB" while the runtime contract is PNG, JPEG and WebP at a 10 MiB ceiling. The sheet reuses the sentence the desktop panel already derives from those two constants, so WebP is not quietly dropped from the product to match an illustration
APP3-S11 UNDO = TOOLBAR_ONLY_NO_MOBILE_LIST — 610:242 carries ↶ and ↷ driving the accepted APP3-S08 controller, with the same bound, the same cursor and the same selection reconciliation. No second history store, and no mobile history list: none of the six approved frames draws one
APP3-S11 CONFLICT_SHEET = S10_DECISION_RELOCATED — 610:514 is where the 610:118 decision appears at 390, so the desktop region is suppressed at that tier rather than joined by a second one. Exactly the two authorized actions, no merge, no third button, and no new retry or revision logic. It cannot be dismissed: APP3-S10 offers a conflict exactly two ways out and neither is "later", so the sheet has no close control and its scrim does not close it
APP3-S11 SHEETS = ONE_PRIMITIVE_FIVE_CALLERS — scrim, focus entry, Tab and Shift+Tab containment, Escape, focus returned to the invoking control, background scroll locked, cleanup on unmount. The scrim is a `button` rather than a clickable div because the feature bans those and the ban is right; it is aria-hidden and out of the tab order, because a second unnamed way to close is not something to announce
APP3-S11 SHELL_390 = ONE_TOPBAR_ONE_CHIP_ONE_STAGE_ONE_TOOLBAR — zero desktop rail, zero right drawer, and the four "this screen is too small" sentences are gone rather than reworded: each said a capability was unavailable at 390 and each would now be false
APP3-S11 MOBILE_ONLY_BY_CONSTRUCTION = REACT_NOT_CSS — every mobile surface is mounted only at the mobile tier, so none of it exists to be reached by a keyboard on a desktop. Touch suppression is scoped to the stage surface while the arbitration is bound to it; the page outside the stage keeps scrolling
APP3-S11 SURFACE = 37 paths / 42 operations / 84 schemas — unchanged; no backend work and no client regeneration
APP3-S11 MIGRATION = NONE
APP3-S11 DEPENDENCY = NONE
APP3-S11 ROOT_SCRIPTS = 30
APP3-S11 BROWSER_PROOF = NINE_OF_ELEVEN_JOURNEYS_ON_A_REAL_TOUCH_SESSION — Playwright against /san-pham/a03-live-check-redirect/thiet-ke over the APP3-S01 trustworthy origin, in a Chromium context with hasTouch, isMobile and deviceScaleFactor 3 at 390×844, driving real CDP touch contacts rather than synthetic pointer events. (A) shell: exactly 1 mobile toolbar, 1 topbar, 0 rail, 0 drawer, 1 stage, 1 <svg>, 0 <canvas>, the save chip reading Đã lưu, 107 watermark tiles, no horizontal overflow, touch-action none on the stage viewport and auto on body; the five toolbar controls measured 55×56 each and each carries a word. (B) one finger on the artwork moved the design, enabled Hoàn tác and produced exactly one PUT, chip Đã lưu lúc 16:30. (C) a two-finger pinch moved the zoom 100% → 200% — one discrete step — with the document object unchanged, no PUT and no history entry; a two-finger pan followed. (D) the selection drew four corner knobs and zero rotate handle; a knob opened 610:294; rotation 0° → 1°; every stepper measured 44×44; growing the width one millimetre at a time reached the APP3-P02 refusal "Không thể đặt đối tượng ra ngoài vùng thêu cho phép." at 85.3 mm and a further press changed nothing. (E) the layer sheet drew the ⋮⋮ handle and both step buttons and zero group controls. (F) the text sheet drew the value field and the size control, reported no font-unavailable, offered no "thêm chữ", and its edit saved. (G) the image sheet's file input carried accept="image/png,image/jpeg,image/webp" and the note "Nhận PNG, JPEG, WebP, tối đa 10 MB" — the B06B reconciliation, live. (H) toolbar undo and redo restored the document with the zoom and the watermark untouched. (J) after a cancelled two-finger gesture, one finger moved the camera by nothing, body scroll was free and no sheet was left behind. At 1024: 0 mobile toolbar, 0 hint, 0 sheet, 1 rail, 1 drawer, 1 topbar, 1 stage, touch not bound
APP3-S11 BROWSER_PROOF_GAPS = CONFLICT_JOURNEY_AND_1440_TIER_NOT_REPRODUCED — the conflict journey (I) did not produce a 409 in the run: the mobile tab had already reconciled to the newer revision before its own edit, so the two-client race did not land. The 1440 regression check and a retry both needed a further anonymous Session, and the IMP-D043 PO-07 window held 8 of 5 for the hour. The run did not restart the API to clear the counter and did not sleep out the window, so both are recorded as not reproduced rather than claimed. Both are covered by the jsdom component suites — the conflict sheet, its two actions and its undismissability by studio-mobile-sheets.test.tsx, and "renders no toolbar, sheet or gesture surface at 1024 or 1440" by the same file
APP3-S11 BENCHMARK = TOOL_DELIVERED_MEASUREMENT_NOT_RUN — tools/bench-app3-s11-touch.mjs ships and is registered as CMD-BENCH-APP3-S11: it measures tap, one-finger move, pinch, two-finger pan, sheet open/close and layer reorder on the accepted S/M/L scenes in an emulated Chromium touch context, and reports against the frozen ADR-APP0-001 20 ms p95 rather than a looser mobile budget it invented. It was not run: every scene needs one anonymous Session and the PO-07 hourly window was exhausted by the journeys above. No number is estimated or carried over from another checkpoint. The desktop L=100 regression §20 asks for is unrun for the same reason
FU-APP3-S11-BENCHMARK-01 = COMPLETE — CLOSED_BY_APP3-E01 — measured with CMD-BENCH-APP3-S11 on the accepted S/M/L scenes at 390x844 in a Chromium context with hasTouch, isMobile and deviceScaleFactor 3, one Session per scene inside the IMP-D043 PO-07 limits. p50/p95 ms per interaction. S (10 elements, 131 DOM nodes): tap 16.4/17.7, move 16.7/16.7, pinch 16.7/16.7, pan 16.7/16.7, sheet 16.7/17.6, reorder 16.3/17.7. M (50, 211): tap 16.5/18.1, move 16.7/16.7, pinch 16.7/16.8, pan 16.7/16.7, sheet 16.7/22.0, reorder 16.4/17.6. L (100, 310): tap 16.5/18.0, move 16.7/16.7, pinch 16.7/16.8, pan 16.7/16.7, sheet 16.6/35.5 with 3 dropped frames, reorder 17.3/18.2. Every GESTURE p95 is within the frozen ADR-APP0-001 20 ms reference at every scene, and none degrades with scene size. The one figure above it is sheet open/close at M and L, which is a modal mount and unmount rather than a transform frame and is therefore not what the 20 ms budget governs; it is recorded rather than excused, and is the mobile surface most worth optimising next. Emulated touch, stated as emulation: no physical device was used
FU-APP3-TRANSFORM-BUDGET-01 = OPEN_WITH_EXPLICIT_ACCEPTED_PHASE_DEBT — CLOSED_AS_INVESTIGATION_BY_APP3-E01. Measured with CMD-BENCH-APP3-S03 at the production cap L = 100 elements (332 DOM nodes, 8 handles) on APP3-S11-accepted HEAD, both engines, the same machine, one Session each inside the IMP-D043 PO-07 limits. p50/p95 ms per gesture frame, dropped frames in the run. Chromium: move 16.7/33.4 dropped 4, resize 16.7/33.3 dropped 3, rotate 16.7/33.4 dropped 3. WebKit: move 47/48 dropped 55, resize 39/48 dropped 34, rotate 46/48 dropped 40. Both exceed the frozen ADR-APP0-001 20 ms p95 desktop reference at L = 100, and the phase accepts that as debt rather than claiming the budget met. ATTRIBUTION = PER_ELEMENT_REACT_SUBTREE_AT_L100_PLUS_WEBKIT_WHOLE_SVG_RERASTER, not a regression introduced by any single checkpoint: APP3-S08 measured L=100 Chromium back to back against APP3-S09-accepted HEAD and found them identical, APP3-S03-C1 had already isolated the cost as 100 React subtrees rather than the geometry authority (0.2 ms of a 30 ms frame), and APP3-S10 adds no per-frame work because its whole state machine is refs. No later capability checkpoint is the cause, so the bisect FU-APP3-TRANSFORM-BUDGET-01 asked for has no interval left to search and is not run. MEASUREMENT_LIMIT = VSYNC_QUANTIZED_60HZ — a rAF-sampled p95 on a 60 Hz display cannot land between 16.7 and 33.4 ms, so the Chromium tail is read honestly as ONE dropped frame in 3-4 of the run rather than as a 33 ms frame cost; the true Chromium figure is somewhere in (16.7, 33.4]. The WebKit figures carry no such ambiguity: 39-48 ms is three sustained vsync intervals with the majority of frames dropped, which is the whole-SVG re-rasterisation on scale change that ADR-APP0-001 already records as the engine cost of the native-SVG decision. REMAINING_WORK = optimise the per-element render (memoised element subtrees, or a transform applied to a group rather than to each element) and re-measure. NOT_BLOCKING_X01: it is a frozen non-functional reference measured and reported with exact values, not an unmet acceptance criterion of any delivered checkpoint.
FU-APP3-STUDIO-TOOL-RAIL-01 = OPEN — NONBLOCKING — AUDITED_BY_APP3-E01_AT_1440_1024_390, still open. Measured in the cross-layer run rather than reasoned about: at 390 the persistent desktop rail is absent (studio-history-rail count 0) and APP3-S11 gives the five approved tools one 56 px toolbar of their own, which is 610:242 and is correct; at 1024 the text tool is still reached through the APP3-S05-C1 drawer trigger, not the rail; at 1440 undo and redo are in the rail and the text and image tools are still panel surfaces. So the gap 609:147 and 618:140 draw is exactly the desktop and tablet one, unchanged by S10 and S11, and no checkpoint has yet owned moving it. APP3-E01 does not close it: an acceptance journey measures the product, it does not recompose an approved surface. NOT_BLOCKING_X01 — every tool is reachable at every viewport; the follow-up is about which surface holds it.
FU-APP3-TEMPLATE-SOURCE-ASSET-INTAKE-01 = DEFERRED_LATER_APP3 — RESTATED_BY_APP3-E01 as the register’s current word. Production Template artwork intake still has no owner. APP3-E01 sources Template bytes through fixtures, which is a harness capability and not an intake path, so the checkpoint deliberately does not close it — and does not let a fixture stand in for the feature.
FU-APP3-UPLOAD-REVISION-SEAM-01 = OPEN — BLOCKS_X01 — FOUND_BY_APP3-E01, product defect, reproduced in every run. An image upload leaves the customer’s next save in conflict. APP3-B06B advances the Session revision as part of the upload — the API states it plainly ("association and revision advance describe one logical fact") and returns the new revision — and APP3-S06 keeps that revision in use-studio-image’s own local state for its NEXT upload without ever handing it to APP3-S10, whose serverRevision still holds the pre-upload value. The autosave that follows is a stale write, the server refuses it 409, and a customer with ONE tab open is shown "Xung đột" for a conflict with nobody. EVIDENCE: the upload journey reports chip "Xung đột" where "Đã lưu" is required, the API log shows exactly one "route":"/api/public/design-sessions/:sessionId/document","statusCode":409 per upload, and the reloaded Session then renders no image href at all because the branch the customer was shown was never the one that persisted. RULED OUT: a second Session credential in the browser — the same failure reproduces on a Session opened in a browser that has held no other. WHY NEITHER CHECKPOINT SAW IT: APP3-S06 was accepted before APP3-S10 existed, and APP3-S10 was accepted against an INJECTED revision rather than one an upload had really moved. It is the exact class of defect a cross-layer journey exists to find. APP3-E01 reports it and does not repair it: the fix is a wiring change inside two accepted capability checkpoints and needs its own tests and review. BLOCKS_X01 because it is a customer-visible correctness defect in a delivered capability, not a measurement gap.
FU-APP3-SESSION-CREDENTIAL-ACCUMULATION-01 = OPEN — NONBLOCKING — FOUND_BY_APP3-E01. Declining a resume and opening a new Session leaves the previous __Host- credential in the browser: measured 2 after one restart, where 1 is intended. APP3-B07 names each cookie after the Session it belongs to, so nothing clears the old one, and the count grows with every restart in a browsing session. No exploitation path was found — each cookie is host-only, Secure, HttpOnly and only unlocks the Session named in it, and the run confirmed the upload defect above is NOT caused by it — so this is recorded as a hygiene defect rather than a security one: a browser accumulating credentials for Sessions the customer has abandoned is state nobody clears until the cookies expire.
FU-APP3-WORKER-BOOT-ORDER-01 = OPEN — NONBLOCKING — FOUND_BY_APP3-E01, environment robustness. The worker fail-fasts on Postgres 57P03 ("the database system is starting up") at boot, closes its pool and exits its startup validation, while the container remains reported "Up". Observed at 05:20 on 2026-08-13: every Session upload from 05:36 onward sat at INSPECTING for six hours with no consumer, and the first upload journey therefore reported a delivery failure that was a dead worker. A restart drained the backlog immediately (ACCEPTED + NORMALIZED READY). The APP3-E01 orchestrator now recreates the worker before any journey depends on it, which makes the RUN reliable and leaves the underlying startup fragility for an owner: "Up" should not be reachable from "gave up on the database".
APP3-E01 = COMPLETE — AWAITING_REVIEW
APP3-E01 RESULT = DELIVERED_WITH_FINDINGS — two complete runs of CMD-JOURNEY-APP3-E01 produced identical results: 52 facts held, 5 failed. Every failure is one customer-visible defect, FU-APP3-UPLOAD-REVISION-SEAM-01, found by putting two accepted checkpoints in the same browser for the first time. The run is not green and is not claimed to be: an acceptance journey reports what the system does.
APP3-E01 SECURITY_FIX = EPHEMERAL_NETWORK_KEY_TAKEN_FROM_TRUSTED_HOP — a caller could choose its own IMP-D043 PO-07 rate bucket by sending X-Forwarded-For; measured 429 for the honest caller and 201 for the forged one before the fix, 429 for every forged attempt after it
APP3-E01 CAPACITY_MECHANISM = ASSIGNED_RUNNER_ADDRESSES — one forwarder container per journey with its own address and loopback port; no forged header, no waited-out window, no API restart
APP3-E01 CONFLICT_FINDING = STALE_WRITE_UNREACHABLE_FROM_THE_UI — a second tab, a second tab denied focus, and a real second HTTP client that advanced the Session server-side all left the customer save succeeding, because APP3-S10 reconciles before every attempt
APP3-E01 DISCLOSED_DEVIATION = HARNESS_FILES_ABOVE_THE_450_CHECKER_CAP — smoke-app3-e01-studio.mjs 559 and smoke-app3-e01-journey.mjs 480; the checker (366) and its tests (316) are within theirs
APP3-E01 EMULATION = TOUCH_EMULATED_390x844_NO_PHYSICAL_DEVICE
APP3-X01 = BLOCKED_BY_FU-APP3-UPLOAD-REVISION-SEAM-01
NEXT_RECOMMENDED_CHECKPOINT = A_CORRECTION_OWNING_FU-APP3-UPLOAD-REVISION-SEAM-01 — not APP3-X01, which cannot close a phase over a customer-visible defect

FU-APP3-UPLOAD-REVISION-SEAM-01 = COMPLETE — CLOSED_BY_APP3-E01-C1 — repaired where the defect was, which was ownership rather than arithmetic. Both Session mutations are compare-and-set on ONE number — APP3-B08 autosave and APP3-B06B upload — and each capability kept its own copy of the bootstrap revision and advanced only that, so an upload advanced the Session and the save that followed still presented the pre-upload revision: 409, and a customer with one tab open shown "Xung đột" for a conflict with nobody. FIX = ONE_SESSION_REVISION_AUTHORITY (use-studio-session-revision.ts), created once by the Studio stage screen above the tier that swaps panels, read by both capabilities when they compose a request and reported to by both when a response carries an authoritative revision. The two private replicas are gone: use-studio-image no longer holds localRevision/boundRevision and use-studio-autosave no longer holds a serverRevision ref. It is a frontend wiring change only — no APP3-B06B contract, backend, worker, database, OpenAPI or generated-client change, and no change to S10 cadence, retry ladder, conflict semantics or S08 history. INVARIANTS: the revision presented is always one a response reported and never revision + 1; adoption is monotonic, so an older answer landing after a newer one cannot lower it; an adoption names the Session it was measured against, so a response for a Session the customer has left is ignored; and adopting a revision renders nothing, so it cannot mark the document clean, clear the past, replace the working document or arm a save — the placement of the image element does that, through the ordinary APP3-S03 commit seam. PROVED BY studio-upload-revision-seam.test.tsx, 10 cases driving the real StudioStageScreen through the real file input and the real autosave loop; the four that pin the handoff were run against the unfixed code first and fail there. RACE RULED: a save can be in flight when an upload returns a newer revision. The frontend rule is monotonicity, and nothing more is added — two concurrent compare-and-set mutations are serialised by the server, which refuses the loser, and each capability already handles that refusal (S10 reconciles, S06 reports a retryable refusal). No queue, no framework, no change to the backend one-in-flight policy.

APP3-E01-C1 = COMPLETE — REVIEW_DELIVERED
APP3-E01-C1 SCOPE = FRONTEND_REVISION_HANDOFF_ONLY — 4 source files changed (one new hook, two hooks, one screen) plus one focused test suite and the gate rule that protects the seam
APP3-E01-C1 VALIDATION = IMPACT_BASED — the changed modules' own suite (10/10) and every suite that renders the changed screen (35 suites, 758 tests), storefront typecheck, the E01 gate and its mutation tests. The full E01 journey, the benchmarks, the browser matrix and the unrelated predecessor gates were NOT rerun: the correction changes none of the source that produced their accepted evidence
...
STUDIO_FRONTEND_BRANCH = S01 + S02 + S07 + S03 + S05 + S06 + S04 + S09 + S08 DELIVERED
