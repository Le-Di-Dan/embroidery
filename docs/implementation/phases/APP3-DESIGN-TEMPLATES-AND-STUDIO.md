...
APP3-S01 SELECTION = IMP-D041_DETERMINISTIC — first row by displayOrder, then code, then id; a leading Side with no Area is skipped; Areas resolve inside their Side only; a Side change re-derives Area and clears Template, preview and cursor in one reducer transition
