...
APP3-S03 PERFORMANCE = MEASURED_16_OF_18_WITHIN_BUDGET — Chromium and WebKit, three production scene sizes (10, 50, 100), three gestures, one pointer move per animation frame against the frozen ADR-APP0-001 §6 transform budget of 20 ms p95 desktop. Chromium 9/9 at p50 16.7 and p95 16.7–16.8 with zero dropped frames. WebKit p50 15–16 everywhere and zero dropped frames everywhere after one bounded optimization pass, with two p95 values still above budget: resize 30 ms at M and 21 ms at L. Move and rotate meet the budget at every size on both browsers
APP3-S03 PERFORMANCE_PASS = ONE_BOUNDED_PASS_SPENT — the element graph was being rebuilt three times a frame over the whole document (the candidate authority, the transform chrome and the physical read-out each built their own); it is now built once per document and shared. WebKit p50 fell 19–26 to 15–16 and dropped frames 3–7 to 0 at every size. The remaining repair is on §22's allowed list rather than a renderer-architecture reopening — memoizing RenderableElement identity so an unchanged element skips its re-render — so §22's BLOCKED condition is not met; the pass budget is spent and the breach is disclosed rather than optimized further or passed over
