...
APP3-S08 HISTORY_LIST = INFORMATIONAL — rows say what happened and whether it is currently applied. No click-to-time-travel: APP3-S08 §14 rules that the presence of a list is not authority for arbitrary jumping, and the two approved frames name states that stepping with the two controls already produces
APP3-S08 MOBILE = NONE_PRE_S11 — 390 renders no undo surface, only the honest sentence the other capabilities render. At 1024 the panel is one more section of the accepted 618:140 drawer, so there is still one topbar, one trigger and one drawer
...
FU-APP3-S08-DESIGN-READ-01 = OPEN — NONBLOCKING — the two APP3-S08 frames were approved from FIGMA_DESIGN_INDEX.md and APP3-D01 prose without opening 609:147 and 609:209, because the Figma MCP connector needs interactive OAuth. The 1024 and 390 compositions and the 1440 panel position are engineering placements to be confirmed against those nodes
