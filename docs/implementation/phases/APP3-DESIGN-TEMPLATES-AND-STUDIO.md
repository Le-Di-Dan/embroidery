...
APP3-S05 FONT_STATES = LOADING_READY_UNAVAILABLE — useControlledFont asks the CSS Font Loading API and reports all three honestly. A failed controlled font is stated in text and never becomes a different authoritative font silently, which is the closest a renderer gets to the registry's locked REJECT_IF_CONTROLLED_FONT_UNAVAILABLE policy
