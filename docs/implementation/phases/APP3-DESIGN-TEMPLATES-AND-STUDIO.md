...
FU-APP3-TEMPLATE-SOURCE-ASSET-INTAKE-01 = OPEN — production Template image intake still has no owner; APP3-S06 did not implement it
...
FU-APP3-S01-FIXTURE-IDEMPOTENCY-01 = OPEN — NONBLOCKING — tools/smoke-app3-s01-fixtures.mjs seeds design_templates with on conflict (id) do nothing, so re-seeding after a revert cannot restore PUBLISHED and the Studio picker lists nothing to clone. It belongs to the APP3-S01 tool rather than to a Studio correction
...
FU-APP3-S10-RETRY-EXTRA-ATTEMPT-01 = OPEN — NONBLOCKING — ROUTED_TO_APP3-E01 — a failing streak that begins with a customer edit performs one extra save attempt one debounce after the first failure, before the locked [5000, 15000] ladder runs from the second. Measured in the browser, bounded, stopping, and reconciled like every other attempt; not reproduced by the fake-clock unit suites. Needs a controlled reproduction that identifies which scheduler armed it before anything is changed. APP3-S11 may not repair it and did not
...
FU-APP3-S11-BENCHMARK-01 = OPEN — NONBLOCKING — the APP3-S11 touch benchmark and the paired desktop L=100 regression have a delivered tool and no measurement, because the IMP-D043 PO-07 Session window was exhausted and this checkpoint would not defeat the guard to get capacity. One run of CMD-BENCH-APP3-S11 in a fresh window closes it
NEXT_RECOMMENDED_CHECKPOINT = APP3-E01
