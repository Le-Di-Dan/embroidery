...
| `CMD-TEST-APP3-B01-INTEGRATION` | `@embroidery/api` | placement live-database suites | `pnpm --filter @embroidery/api exec jest --runInBand test/integration/product-placement.integration.spec.ts test/integration/product-placement-manifest.integration.spec.ts` | apps/api/test/integration/product-placement*.integration.spec.ts | Any change to the placement repository, transaction, retirement/supersession behaviour or the `studioEligible` derivation. Provisions a **disposable** PostgreSQL database and applies every migration; needs a running dev PostgreSQL container, no object storage. | `ACTIVE_SCOPED` |
