...
- **Sass resolution method:** each app imports the package by name (`@use '@embroidery/styles' as styles;`). Next's Sass loader resolves the package entry via the package `exports`/`sass` field, but cannot resolve the package's internal directory-index partials (`@forward 'settings'`); the documented Next mechanism `sassOptions.loadPaths` was added to each `next.config.ts`, pointing at `packages/styles/src`, so the foundation's internal `@use`/`@forward` graph resolves. This is the only change outside the checkpoint's default allowed-file list; it is essential build configuration (both builds fail without it) and touches neither the shared package source nor any screen. No app SCSS imports `@embroidery/styles/src` internals.
...
`PASS_WITH_FOLLOW_UPS` — both apps integrate the shared foundation and build successfully, a cross-platform styling-boundary guardrail is enforced and wired into `quality`, and no prohibited styling was introduced. Follow-up: the `sassOptions.loadPaths` addition to each `next.config.ts` (necessary, outside the default allowed-file list) is flagged for reviewer awareness; and full `pnpm quality` should be run in a PostgreSQL-capable environment at phase closure.
