...
|---|---|---|---|---|
| `FU-APP11-S04-C1-02` | `S04-C1` | `PublicCategoryResponse.slug` is the closed enum `{thu-bong, khan, quan-ao, khac}` in the committed contract, but `categories.slug` is unconstrained `text` and the runtime emits `ao-thun` | **`CLOSED` — contract half by `APP12-C01`, Storefront half by `APP12-C01-C1`.** Discover, the Product breadcrumb, the continuation CTA and the sitemap category URLs are all served from `categories` rows at request time; both `legacy-category-slugs.ts` shims and the Admin slug-to-label map are deleted, and `tools/check-category-source-of-truth.mjs` fails the build if any of it returns. `ao-thun` renders its chip, its breadcrumb crumb, its filtered feed and its sitemap URL with no source edit. `APP12-C03` remains open as the final dynamic-category SEO/route-authority **verification** checkpoint, and `APP12-A01` as the category-management UI owner — neither is a remaining part of this follow-up. *(Superseded detail, kept for history:* **Contract half `CLOSED_BY_APP12_C01`** — `PublicCategoryResponse.slug` and every other category seam now publish the pattern `^[a-z0-9]+(?:-[a-z0-9]+)*# APP11 — Closure Matrix
...
| ID | Origin | Description | Class | Owner / next authority |
|---|---|---|---|---|
, the four generated enums are gone, and `GET /api/public/categories` serves the taxonomy at runtime, so the published contract no longer forbids a slug the runtime emits. **Storefront half remains open, owner `APP12-C03`**: Discover, the Product Detail breadcrumb and the static sitemap category routes still use the four historical slugs (behind `apps/storefront/src/features/product-discovery/model/legacy-category-slugs.ts`, whose removal owner is `APP12-C03`), so a fifth category is still absent from Discover and still drops its breadcrumb crumb. The Admin counterpart shim is owned by `APP12-A01`. )* | Closed: `APP12-C01` + `APP12-C01-C1`. Verification: `APP12-C03`. Category-management UI: `APP12-A01` |
