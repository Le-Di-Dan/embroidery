...
| REQ-CAT-003 | Product publication/archival is an admin operation (create/edit/publish/unpublish/archive, out-of-stock, display order). Unpublish returns a published product to the editable draft state and is distinct from archive (TR-LC04-05, IMP-D035). | `07 §3` | LOCKED | Product Publication State | Catalog | public | mutable | L,A | DB3 | Publication lifecycle. |
