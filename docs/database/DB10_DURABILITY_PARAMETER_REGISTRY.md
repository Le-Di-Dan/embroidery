...
|---|---|---|---|---|---|---|
| DP-RET-01 | `trans` — transient | `design_sessions` (+assets), `contact_verification_challenges` (+attempts), `outbox_events`, `idempotency_records` | `design_sessions`: **30 days absolute from `created_at`**, then `EXPIRED`, then hard delete after a **24-hour** grace. Other tables in the class: *none set* | `design_sessions` **LOCKED — `APP3-G03` / IMP-D043 PO-06 (closes `O-008`)**; remainder **DEFERRED — business** | business/operations; `design_sessions` ruled by the Product Owner | mechanism rehearsed in DB10-CP4 with a caller-supplied cutoff — the cutoff for `design_sessions` is now `created_at + 30 days`, **not** `last_activity_at` |
