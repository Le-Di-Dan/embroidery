...
| CST-129 | CK | design_versions | exactly one placement branch: complete Catalog quartet **and** `customer_owned_product_id IS NULL`, XOR all four Catalog columns NULL **and** `customer_owned_product_id IS NOT NULL` (`ck_design_versions__exactly_one_placement_branch`) | **ADR-APP6-001 §3.2/§4.1, INV-13** | a customer's own garment fabricated into a catalogue SKU to satisfy four `NOT NULL` FKs — or a half-populated placement, rejected by the same expression | **yes** (APP6-DB01, migration 0036) | D7 (`app6-cop-design-context.integration.spec.ts`) |
| CST-130 | CK | design_versions | COP placement labels: `customer_owned_product_id IS NULL` **and** both labels NULL, XOR `customer_owned_product_id IS NOT NULL` **and** both labels nonblank once space, tab, CR and LF are trimmed (`ck_design_versions__cop_placement_labels`; `btrim` carries an explicit character set because the bare form trims spaces only) | **ADR-APP6-001 §3.7** | a COP approval with no truthful side/area label, when `approval_snapshots` and `production_specifications` both require one `NOT NULL` downstream | **yes** (APP6-DB01, migration 0036) | D7 (`app6-cop-design-context.integration.spec.ts`) |
| CST-131 | CK | approval_snapshots | exactly one placement branch, the CST-129 truth table without the label clause (`ck_approval_snapshots__exactly_one_placement_branch`) | **ADR-APP6-001 §3.2/§4.1, INV-13** | approval evidence naming a product the customer does not own — worse than evidence naming nothing | **yes** (APP6-DB01, migration 0036) | D7 (`app6-cop-approval-snapshot.integration.spec.ts`) |
