#!/usr/bin/env node
/**
 * `APP2-S02` §25 — isolated production evidence for the Storefront Product
 * Detail page.
 *
 ...
 *
 * The topology is the accepted `APP2-T01-C1` / `APP2-B04` / `APP2-S01` one,
 * imported rather than reimplemented: the same canonical Dockerfiles, the same
 * temporary Compose override outside the repository, the same disposable TLS
 * PostgreSQL (a production API may not attach to the development database — its
 * TLS and password guards correctly forbid it), the same bounded waits, the same
 * structural `finally` restore and the same gateway reload after every swap and
 * after the restore. `APP2-S02` changes only which scenarios run against it.
 *
 ...
 *
 *   pnpm smoke:app2-s02-detail:production
 */
...
const BROWSER_SCRIPT = join(REPO_ROOT, 'tools', 'smoke-app2-s02-detail-browser.mjs');
...
function runScenarios() {
  ...
  record('production Product Detail browser scenarios', scenarios.status === 0, {
...
function buildImages({ imageTag, storefrontTag, databaseImageTag, pgContextDir }) {
  ...
  record(
    ...
    storefront.status === 0,
    { tag: storefrontTag, exit: storefront.status },
  );
...
async function main() {
  ...
  const databasePassword = `s02${randomBytes(18).toString('hex')}`;
  const dir = mkdtempSync(join(tmpdir(), 's02-'));
  ...
  console.log('== APP2-S02 isolated production Product Detail smoke ==');
