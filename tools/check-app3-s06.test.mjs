...
describe('the artifacts and the command index', () => {
  ...
  /*
   * Autosave crossed the boundary at `APP3-S10`, which is the screen that owns
   * saving — the exact condition its withholding stated. So the mutation is
   * retargeted at an operation that is still withheld rather than deleted: the
   * rule is about *withholding until a consumer exists*, not about autosave.
   */
  it('refuses a still-unowned operation crossing the curated boundary', () => {
    const curatedClient = `${file('curatedClient')}\npublicProductMediaGet,\n`;
    assert.ok(mentions(run(checkImmutability, { curatedClient }), 'publicProductMediaGet'));
  });
