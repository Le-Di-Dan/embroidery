...
/**
 * `APP3-B04A` — the one restore operation (`TR-LC24-06`).
 *
 * A new tier, and no line above it edited: every earlier world still proves the
 * artifact that phase published, so a rollback is checked against what it
 * actually shipped rather than against the newest numbers.
 */
const OPENAPI_SHA256_AFTER_B04A =
  'f736306045faa1c7a638bf7749b27051e351b967d29d574c20c6dce1fb581908';
const CLIENT_TREE_SHA256_AFTER_B04A =
  'f47c774ad5a6b3e1f40ad1d1413e6693e3a7ba5a1088f7cadd716a17519f62d9';
const OPENAPI_FACTS_AFTER_B04A = Object.freeze({ paths: 34, operations: 39, schemas: 83 });

/**
 * `APP3-S06` — the Studio image capability, and the one narrow read it needed.
 *
 * A frontend checkpoint with a contract delta, which is unusual and is exactly
 * why it is recorded here: the accepted design needs a state `APP3-B06C`
 * deliberately cannot report, because that route answers one indistinguishable
 * 404 for every private miss. One status projection closes it.
 *
 * Schemas move to 84 — the projection is the one component published — and both
 * numbers are measured from the artifact rather than predicted. A new tier, and
 * nothing above it edited, for the reason every tier here exists: each earlier
 * world still proves the artifact *that* phase published.
 */
const OPENAPI_SHA256_AFTER_S06 = 'f39e9e8fca1de08417359aeb299b874c346e21902b21375c9159124ed63d3817';
const CLIENT_TREE_SHA256_AFTER_S06 =
  'c2fb229f69f4d0033b081e7f2aca7328653c5eeb6bebfb5658feb6fe8ba4a2d6';
const OPENAPI_FACTS_AFTER_S06 = Object.freeze({ paths: 37, operations: 42, schemas: 84 });

/**
 * `APP3-B06C` — the one contextual private Design Session asset delivery.
 *
 * A new tier, and nothing above it edited, for the reason every tier here exists:
 * each earlier world still proves the artifact *that* phase published, so a
 * rollback is checked against what it actually shipped rather than against the
 * newest numbers.
 *
 * Schemas stay at 83: the response is binary and the request has no body, so the
 * operation publishes a media type rather than a component. Measured, not
 * assumed — a count that happens to match its predecessor is still a
 * measurement.
 */
const OPENAPI_SHA256_AFTER_B06C =
  '0bdb5261acc233aa33b21972c2069157319923c52beaed03c9bbb7f5e3867307';
const CLIENT_TREE_SHA256_AFTER_B06C =
  'd8ff2e10bfa5335bd0b86ac1e482fc32eb9da96c31e2f5107d78328be755b8d8';
const OPENAPI_FACTS_AFTER_B06C = Object.freeze({ paths: 36, operations: 41, schemas: 83 });
 ...
 *
 * Schemas stay at 83: the response is binary and the request has no body, so the
 * operation publishes two media types rather than a component.
 */
const OPENAPI_SHA256_AFTER_B05A =
  '592b9bf78d6dbdea89a403f7509ee7c6ccac2390618432f74556b334d6b7c435';
const CLIENT_TREE_SHA256_AFTER_B05A =
  'd9aac2b3bfb322c1d604f2802e8a9b154bcb8744eabc780652658196d84735b9';
const OPENAPI_FACTS_AFTER_B05A = Object.freeze({ paths: 35, operations: 40, schemas: 83 });

/** * `APP3-B03B` — the one initial scope-assignment operation.
 *
 * A new tier rather than an edit to any line above: every earlier world still
 * proves the artifact *that* phase published, so a rollback is checked against
 * what it actually shipped rather than against the newest numbers.
 */
const OPENAPI_SHA256_AFTER_B03B =
  '1aa6e64709bd1f28a46bd83a8b4e2597926f7562e71f82032cda291e90e9d874';
const CLIENT_TREE_SHA256_AFTER_B03B =
  '96c3a6a59f19278a5de8b581d2d2d1d7cf8d8d4936a4e247548017e4adb8d618';
const OPENAPI_FACTS_AFTER_B03B = Object.freeze({ paths: 33, operations: 38, schemas: 82 });
/**
...
export function checkApp3B01NArtifacts(rootDir, fail) {
  ...
  const afterS06 = isS06Delivered(rootDir);
  const afterB06C = afterS06 || isB06CDelivered(rootDir);
  const afterB05A = afterB06C || isB05ADelivered(rootDir);
  const afterB04A = afterB05A || isB04ADelivered(rootDir);
  const afterB03B = afterB04A || isB03BDelivered(rootDir);
  const afterB02A = afterB03B || isB02ADelivered(rootDir);
  ...
  const facts = afterS06
    ? OPENAPI_FACTS_AFTER_S06
    : afterB06C
      ? OPENAPI_FACTS_AFTER_B06C
      : afterB05A
        ? OPENAPI_FACTS_AFTER_B05A
        : afterB04A
          ? OPENAPI_FACTS_AFTER_B04A
          : afterB03B
            ? OPENAPI_FACTS_AFTER_B03B
            : afterB02A
                          ...
                          : afterB06B
                            ...
                            : afterB07
                              ...
                              : afterB02
                                ...
                                : OPENAPI_FACTS;
  const expectedDigest = afterS06
    ? OPENAPI_SHA256_AFTER_S06
    : afterB06C
      ? OPENAPI_SHA256_AFTER_B06C
      : afterB05A
        ? OPENAPI_SHA256_AFTER_B05A
        : afterB04A
          ? OPENAPI_SHA256_AFTER_B04A
          : afterB03B
            ? OPENAPI_SHA256_AFTER_B03B
            : afterB02A
                            ...
                            : afterB07
                              ...
                              : afterP03
                                ...
                                : afterB02
                                  ...
                                  : OPENAPI_SHA256;
  const expectedTree = afterS06
    ? CLIENT_TREE_SHA256_AFTER_S06
    : afterB06C
      ? CLIENT_TREE_SHA256_AFTER_B06C
      : afterB05A
        ? CLIENT_TREE_SHA256_AFTER_B05A
        : afterB04A
          ? CLIENT_TREE_SHA256_AFTER_B04A
          : afterB03B
            ? CLIENT_TREE_SHA256_AFTER_B03B
            : afterB02A
